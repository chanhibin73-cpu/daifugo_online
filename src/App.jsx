  ...
  useEffect(() => {
    if (isOnlineMode && isHost && roomId && currentScreen === 'game') {
      const stateToSync = {
        ...
        players, playerRatings // ← ここに追加！(同期漏れバグの修正)
      };
    ...
    if (isOnlineMode && !isHost && roomId && currentScreen === 'game') {
      ...
      const unsub = onValue(gsRef, (snap) => {
        ...
        if (val) {
          ...
          // Firebaseでnullや空配列/空オブジェクトが消える仕様への対応（フォールバック）
          ...
          // バグ修正: プレイヤー名とレーティングの同期漏れ解消
          if (val.players) setPlayers(val.players);
          if (val.playerRatings) setPlayerRatings(val.playerRatings);
