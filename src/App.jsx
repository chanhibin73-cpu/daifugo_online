  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      list.forEach((user, idx) => {
        ...
        let newRankTrend = user.rankTrend || 'same';
        let newRankTrendUpdatedAt = user.rankTrendUpdatedAt || now;
        let newLastRank = user.lastRank || currentRank;
        let newRankDiff = user.rankDiff || 0;
        let needsUpdate = false;

        if (user.lastRank && user.lastRank !== currentRank) {
          newRankTrend = currentRank < user.lastRank ? 'up' : 'down';
          newRankDiff = Math.abs(currentRank - user.lastRank);
          newRankTrendUpdatedAt = now;
          newLastRank = currentRank;
          needsUpdate = true;
        }

        let displayTrend = null;
        let displayDiff = 0;
        if (now - newRankTrendUpdatedAt <= 3 * 60 * 60 * 1000) {
          displayTrend = newRankTrend;
          displayDiff = newRankDiff;
        } else {
          if (newRankTrend !== 'same') {
            newRankTrend = 'same';
            newRankDiff = 0;
            needsUpdate = true;
          }
        ...
        user.trend = displayTrend;
        user.trendDiff = displayDiff;
        ...
        if (needsUpdate && user.userId) {
          try {
            updateDoc(doc(db, 'users', user.userId), {
              ...
              rankTrendUpdatedAt: newRankTrendUpdatedAt
            }).catch(()=>{});
          } catch(e) {}
        }
      });
                  ...
                  return (
                    <div key={idx} className={`ranking-item ${user.userId === userData.userId ? 'highlight-me' : ''}`}>
                      <span style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {user.trend === 'up' && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold' }}>↑ {user.trendDiff}</span>}
                        {user.trend === 'down' && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold' }}>↓ {user.trendDiff}</span>}
                      </span>
              ...
              <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '2px dashed #bdc3c7' }}>
                ...
                <div className="ranking-item highlight-me" style={{ margin: '0', borderRadius: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                    ...
                    {myRankingInfo.user.trend === 'up' && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold' }}>↑ {myRankingInfo.user.trendDiff}</span>}
                    {myRankingInfo.user.trend === 'down' && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold' }}>↓ {myRankingInfo.user.trendDiff}</span>}
                  </span>
