...
const UPDATE_HISTORY = [
  {
    version: '2.3.2',
    features: [
      '📊 ランキングの順位変動数を3時間の累計・3時間ごとの更新ロジックに変更しました。'
    ]
  },
  {
    version: '2.3.1',
    features: [
      '🎨 ランキング順位変動の矢印の色を逆転させました（隠れアップデート）'
    ]
  },
  {
    version: '2.3',
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      list.forEach((user, idx) => {
        ...
        let lastPeriodStart = user.lastPeriodStart || now;

        if (now - lastPeriodStart >= threeHoursMs) {
        ...
        user.trend = trend;
        user.trendAmount = Math.abs(diff);
              ...
              <div className="hand-cards">
                {hands[mySlot]?.map(card => {
                  ...
                  return (
                    ...
                    >
                      {userData.cardDesign === 'mark' ? `${card.suit}${NUM_MAP[card.num] || ''}` : (NUM_MAP[card.num] || c.suit)}
                    </div>
                  );
                })}
              </div>
                  ...
                  return (
                    <div key={idx} className={`ranking-item ${user.userId === userData.userId ? 'highlight-me' : ''}`}>
                      <span style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {user.trend === 'up' && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold' }}>↑{user.trendAmount > 1 ? user.trendAmount : ''}</span>}
                        {user.trend === 'down' && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold' }}>↓{user.trendAmount > 1 ? user.trendAmount : ''}</span>}
                      </span>
              ...
              <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '2px dashed #bdc3c7' }}>
                ...
                <div className="ranking-item highlight-me" style={{ margin: '0', borderRadius: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                    ...
                    {myRankingInfo.user.trend === 'up' && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold' }}>↑{myRankingInfo.user.trendAmount > 1 ? myRankingInfo.user.trendAmount : ''}</span>}
                    {myRankingInfo.user.trend === 'down' && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold' }}>↓{myRankingInfo.user.trendAmount > 1 ? myRankingInfo.user.trendAmount : ''}</span>}
                  </span>
