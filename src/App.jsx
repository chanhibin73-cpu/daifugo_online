...
function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(learningRef, (snap) => {
      ...
      if (val) {
        ...
        setAiLearningData(vals.slice(-100)); // 最新100件を保持
      }
    });
  ...
  useEffect(() => {
    const rawAiPlayers = [
      ...
      // ★ Ver3.3.1 AI_asn_v2 に isAsnV2 を追加し、思考力強化・学習対応に
        ...
        if (matchCountSim > 0 && !isRestTime) {
           ...
           for (let mCount = 0; mCount < matchCountSim; mCount++) {
              ...
              playersForMatch.sort((a, b) => {
                ...
                // ★ Ver3.3.1 AI_asn_v2の強さを3000ベースで計算
              ...
              for (let i = 0; i < 4; i++) {
                 if (playersForMatch[i].userId !== 'mob') {
                    ...
                    if (aiTarget) {
                      ...
                      // ★ Ver3.3.1 DoubleGrowthの場合、上がる時も下がる時も変動を2倍に（成長速度2倍の表現）
                      if (aiTarget.isDoubleGrowthAI && change > 0) change *= 2; 
  ...
  // ★ Ver3.3.1 AI_asn_v2 用の高度な思考ルーチンを追加
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, allHands = [], currentTurn = 0, isAIPlayer = false, isSuperAI = false, isUltraSuperAI = false, isDoubleGrowthAI = false, isAsnV2 = false, learningData = []) => {
    ...
    // ★ Ver3.3.1: AI_asn_v2 専用の思考ルーチン（手数を減らす、強いカードを残す、人間のプレイスタイルを反映）
          ...
          if (p && p.isAIPlayer) {
            try {
              ...
              if (aiSnap.exists()) {
                ...
                // ★ Ver3.3.1 DoubleGrowth対応 (10試合制でも5試合制でもレート変動を2倍に)
                if (aiData.isDoubleGrowthAI && aiChange > 0) {
                    aiChange *= 2;
                }
  ...
  const handleUserPlay = () => {
    if (isValidPlay(selectedCards, field, isReversedEffective)) {
      // ★ Ver3.3.1 人間のプレイデータをFirebaseへ送信（AI研究用）
      try {
  ...
  useEffect(() => {
    if (!userData.userId) return;
    const notifRef = ref(rtdb, `notifications/${userData.userId}/unreadChat`);
    const unsub = onValue(notifRef, (snap) => {
      setUnreadChats(snap.val() || {});
    });
    return () => off(notifRef);
  }, [userData.userId]);

  useEffect(() => {
    if (currentScreen === 'chat' && activeChatFriend && userData.userId) {
      remove(ref(rtdb, `notifications/${userData.userId}/unreadChat/${activeChatFriend.userId}`));
    }
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'users', userData.userId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (!isFirstLoadRef.current && reqs.length > prevRequestsRef.current) {
           setFriendRequestPopup('新しいフレンド申請が届いています！');
           setTimeout(() => setFriendRequestPopup(null), 5000);
        }
        prevRequestsRef.current = reqs.length;
        isFirstLoadRef.current = false;

        const rIdsStr = reqs.sort().join(',');
