...
function App() {
  ...
  const playCards = (playerIdx, cards) => {
    ...
    // ★ Ver.1.7.5: 階段の場合は11バックの特殊効果を無効化
    ...
    if (isFinished) {
      // ★ Ver.1.7.5: 階段のときは8を含んでいても効果が出ないため、禁止上がり扱いにもしない
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false, allHands = [], currentTurn = 0) => {
    ...
    const isForbiddenFinish = (cards) => {
      ...
      const isSeq = checkSequence(cards); // CPUの禁止上がり判定時にも特殊無効化を考慮
      const contains8 = !isSeq && cards.some(c => c.num === 8);
      const containsJoker = cards.some(c => c.num === 0);
    ...
    if (isUltraAI || isEnhancedAI) {
      if (someoneIsFinishing) {
        ...
        const noSpecials = availableOptions.filter(combo => {
          ...
          // 階段なら8や11が含まれていても特殊扱いにならないため温存対象にしない
