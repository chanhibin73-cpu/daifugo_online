...
function App() {
  ...
  // ★ ホスト側の同期送信（バグ修正：配列をJSON化して欠落を防ぐ）
  useEffect(() => {
  ...
  // ★ ゲスト側の同期受信（バグ修正：JSONからパースして復元する）
  useEffect(() => {
    if (isOnlineMode && !isHost && roomId && currentScreen === 'game') {
      ...
      const unsub = onValue(gsRef, (snap) => {
        ...
        if (val) {
          ...
          if (val.players) setPlayers([0, 1, 2, 3].map(i => val.players[i] || `CPU ${i}`));
          if (val.playerRatings) setPlayerRatings([0, 1, 2, 3].map(i => val.playerRatings[i] || 500));
