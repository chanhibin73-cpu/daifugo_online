...
// レーティング計算ロジック（Ver.1.6.1: レート高低による補正と800以下の上昇率アップ）
const calcRating = (myRating, allRatings, myRank, finalRankings) => {
  ...
  // 基準レート1000とした補正係数 (低いほど上がりやすく下がりづらい、高いほど上がりづらく下がりやすい)
  let factor = 1000 / Math.max(400, myRating); 
  factor = Math.pow(factor, 0.6); // 緩やかにカーブをかける

  let change = 0;
  if (rawChange > 0) {
    change = rawChange * factor;
    // 低レーティング(800以下)の特別上昇ボーナス
    if (myRating <= 800) {
      change *= 1.5; 
    }
  } else {
    // 減少幅の調整（高レートほど減りやすく、低レートほど減りにくい）
