...
const UPDATE_HISTORY = [
  {
    version: '3.4.1',
    features: [
      '⚙️ CPUの動作安定化および特定の状況下での処理の微調整を行いました。'
    ]
  },
  {
    version: '3.3.6',
  ...
  },
  {
    version: '3.3.5',
    features: [
      '🐛 8切りやスペ3返し時にゲームが進行しなくなる不具合を修正しました。'
    ]
  },
  {
    version: '3.3.4',
...
function App() {
  ...
  const [isReadyForMatch, setIsReadyForMatch] = useState(false);
  ...
  // ★ Ver3.4.1: 自分のターンが回ってきた時、または交換フェーズの時にロックを解除する
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      if (data) {
        setRoomData(data);
        if ((data.status === 'starting' || data.status === 'playing') && currentScreen.startsWith('waiting')) {
  ...
  const startOnlineGameWithCpu = async () => {
    if (!roomId || !roomData || isStartingRoom) return;
    ...
    await update(ref(rtdb, `rooms/${roomId}`), { players: currentPlayers, status: 'starting' });
  };

  useEffect(() => {
    if (currentScreen === 'game' && isOnlineMode && roomData?.status === 'starting' && !isReadyForMatch) {
       setIsReadyForMatch(true);
       push(ref(rtdb, `rooms/${roomId}/readyRequests`), userData.userId);
    }
  }, [currentScreen, isOnlineMode, roomData?.status, isReadyForMatch, roomId, userData.userId]);

  useEffect(() => {
    if (isOnlineMode && isHost && roomId) {
      const readyRef = ref(rtdb, `rooms/${roomId}/readyRequests`);
      const unsub = onValue(readyRef, (snap) => {
        const val = snap.val();
        if (val) {
          setReadyPlayers(Object.values(val));
        } else {
          setReadyPlayers([]);
        }
      });
  ...
  useEffect(() => {
    if (isOnlineMode && isHost && roomId && roomData?.status === 'starting') {
      const nonCpuPlayers = (roomData.players || []).filter(p => !p.isCpu);
      const allReady = nonCpuPlayers.length > 0 && nonCpuPlayers.every(p => readyPlayers.includes(p.userId));
      
      if (allReady && !hasStartedMatch) {
        setHasStartedMatch(true);
        executeStartOnlineMatch(roomId, roomData.players);
      }
    }
  }, [isOnlineMode, isHost, roomId, roomData, readyPlayers, hasStartedMatch]);
  ...
  useEffect(() => {
    if (isOnlineMode && isHost && roomId && currentScreen === 'game' && roomData?.status === 'playing') {
  ...
  const playCards = (playerIdx, cards) => {
    ...
    if ((!isSeq && cards.some(c => c.num === 8)) || isSpade3Return) {
      ...
      if (isFinished) {
        logs.push(actionMsg);
        setMessage(logs.join(' / '));
        // ★ Ver3.4.1: 二重のターン進行を防ぐため、上がりの場合は clearField のみ実行して終了する
  ...
  // ★ Ver3.4.1 CPU用最新データ参照用Ref （フリーズバグ対策）
  const latestDataRef = useRef({ roomData, aiLearningData, playerRatings, hands, isOnlineMode, isHost });
  useEffect(() => {
    latestDataRef.current = { roomData, aiLearningData, playerRatings, hands, isOnlineMode, isHost };
  }, [roomData, aiLearningData, playerRatings, hands, isOnlineMode, isHost]);

  // CPU自動打札
  useEffect(() => {
    if (currentScreen !== 'game' || matchResultModal || gameResultModal || exchangePhase) return;
    
    // ★ 常に最新のデータを参照して、頻繁な再レンダリングでタイマーがキャンセルされるのを防ぐ
    const { roomData: currentRoomData, isOnlineMode: currentIsOnlineMode, isHost: currentIsHost, hands: currentHands, playerRatings: currentPlayerRatings, aiLearningData: currentLearningData } = latestDataRef.current;

    const isCurrentCpuSlot = currentIsOnlineMode ? (currentIsHost && currentRoomData?.players && currentRoomData.players[turn]?.isCpu) : (turn !== 0);

    if (isCurrentCpuSlot) {
      const cpuHand = currentHands[turn];
      if (!cpuHand || cpuHand.length === 0) return;
      
      const isAIPlayer = currentIsOnlineMode && currentRoomData?.players && currentRoomData.players[turn]?.isAIPlayer;
      const isSuperAI = currentIsOnlineMode && currentRoomData?.players && currentRoomData.players[turn]?.isSuperAI;
      const isUltraSuperAI = currentIsOnlineMode && currentRoomData?.players && currentRoomData.players[turn]?.isUltraSuperAI;
      const isDoubleGrowthAI = currentIsOnlineMode && currentRoomData?.players && currentRoomData.players[turn]?.isDoubleGrowthAI;
      const isAsnV2 = currentIsOnlineMode && currentRoomData?.players && currentRoomData.players[turn]?.isAsnV2;
      
      let cpuThinkRating = currentPlayerRatings[turn];
      if (currentIsOnlineMode && currentRoomData?.players && currentRoomData.players[turn]?.maxRating) {
        cpuThinkRating = currentRoomData.players[turn].maxRating;
      }
      
      const move = findBestCpuMove(cpuHand, field, isReversedEffective, cpuThinkRating, currentHands, turn, isAIPlayer, isSuperAI, isUltraSuperAI, isDoubleGrowthAI, isAsnV2, currentLearningData);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [turn, field, currentScreen, isReversedEffective, matchResultModal, gameResultModal, exchangePhase]);
  ...
  const handleUserPlay = () => {
    ...
    if (isValidPlay(selectedCards, field, isReversedEffective)) {
      ...
      if (isOnlineMode && !isHost) {
        ...
        .catch(() => setIsProcessing(false)); // 通信失敗時はロック解除
        setSelectedCards([]);
      } else {
      ...
      if (snapshot.exists()) {
        ...
        if (existingPlayers.length < 4) {
          ...
          if (updatedPlayers.length === 4 && type.startsWith('rank')) {
            await update(ref(rtdb, `rooms/${targetRoomId}`), { status: 'starting' });
          }
        } else {
          alert('部屋が満員です。');
        }
      ...
      <div className="app-container">
        ...
        {currentScreen === 'game' && (
          <div className="game-board">
            {/* ★ Ver3.3.6 ホストの試合開始待機（同期フェーズ）用のローディング */}
            {isOnlineMode && roomData?.status === 'starting' && (
              <div className="modal-overlay" style={{ zIndex: 10000, background: 'rgba(0,0,0,0.8)' }}>
                <div style={{ color: 'white', textAlign: 'center' }}>
                   <h2>通信中...</h2>
                   <p>他のプレイヤーの接続を待機しています ({readyPlayers.length}/{roomData?.players?.filter(p => !p.isCpu).length})</p>
                </div>
              </div>
            )}
              ...
              {!exchangePhase && (
                <div className="controls">
                  <button
                    ...
                    disabled={turn !== mySlot || selectedCards.length === 0 || isProcessing || roomData?.status === 'starting'}
                  ...
                  <button
                    ...
                    disabled={turn !== mySlot || !field || isProcessing || roomData?.status === 'starting'}
