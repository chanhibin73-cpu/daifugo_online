...
function App() {
  ...
  // ★ Ver3.4 二重送信防止用フラグ＆オンラインキュー処理中フラグ
  ...
  // 自分のターンが回ってきた時、または場が流れた時にロックを解除する
  useEffect(() => {
    if (turn === mySlot || exchangePhase) {
      setIsProcessing(false);
    }
  }, [turn, mySlot, exchangePhase, field]);
  ...
  // ★ Ver3.4: キュー処理の排他制御（二重送信・競合バグの防止）
  useEffect(() => {
    if (isOnlineMode && isHost && requestsQueue.length > 0 && !isProcessingQueue) {
      ...
      const processNext = async () => {
        ...
        await remove(ref(rtdb, `rooms/${roomId}/requests/${req.id}`));
        setIsProcessingQueue(false);
      };
      processNext();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [requestsQueue, isOnlineMode, isHost, roomId, isProcessingQueue]);
  ...
  const startMatch = (mCount, lastRanks, playerList = players) => {
    ...
    if (mCount > 1) {
      ...
      if (newHands[firstTurn].length === 0 && activeIdxs.length > 0) {
         let loop = 0;
         while (newHands[firstTurn].length === 0 && loop < 4) {
  ...
  const isValidPlay = (cards, currentField, isRev) => {
    ...
    if (currentField.isBound) {
      ...
      const newSuits = nonJokers.map(c => c.suit);
      ...
      const neededJokers = tempPrevSuits.length;
      const availableJokers = cards.length - nonJokers.length;

      if (!isSameSuit || neededJokers > availableJokers) {
          return false;
      }
    }
  ...
  // ★ Ver3.4: 無限ループ回避用の安全装置(loopCount)を追加
  const clearField = (nextTurnPlayer, currentHands, currentRankings, currentFouled = fouledPlayers) => {
    ...
    let loopCount = 0;
    while (currentHands[next].length === 0 && (currentRankings.length + currentFouled.length) < 3 && loopCount < 4) {
      next = (next + 1) % 4;
      loopCount++;
    }
    setTurn(next);
  ...
  const advanceTurn = (currentIdx, currentHands, currentRankings, currentFouled = fouledPlayers, currentField, currentPassed = passed) => {
    ...
    } else if (currentField && !activePlayers.includes(currentField.playedBy)) {
      // 最後に場に出したプレイヤーがすでに上がっている場合
      if (activePassed.length >= activePlayers.length) {
        let nextStart = (currentField.playedBy + 1) % 4;
        let loop = 0;
        while (currentHands[nextStart].length === 0 && loop < 4) {
           nextStart = (nextStart + 1) % 4;
           loop++;
        }
        clearField(nextStart, currentHands, currentRankings, currentFouled); return;
      }
    }

    if (activePassed.length >= activePlayers.length) {
      clearField(currentIdx, currentHands, currentRankings, currentFouled); return;
    }

    let next = (currentIdx + 1) % 4;
    let loopCount = 0;
    while (currentHands[next].length === 0 && loopCount < 4) { 
        next = (next + 1) % 4; 
        loopCount++;
    }
    setTurn(next);
