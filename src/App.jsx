...
function App() {
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      if (data) {
        ...
        if (data.status === 'playing' && currentScreen.startsWith('waiting')) {
          ...
          // ★修正: ホスト以外のプレイヤーもゲーム開始時にカードを配布する
          if (data.hostId !== userData.userId) {
            ...
            startNewGameSet(playerNames, data.type === 'rank', true);
          }
        }
      }
    });
  ...
  const advanceTurn = (
    ...
    // パスは「1手番スキップ」のみになったため、パスしたプレイヤーも手番が回る
    let next = (currentIdx + 1) % 4;
  ...
  useEffect(() => {
    ...
    // ★修正: オンライン時は他プレイヤーのターンをCPUとして自動進行させる
  ...
  }, [turn, field, hands, currentScreen, isReversedEffective, matchResultModal, roomData, isOnlineMode, cpuLevels, mySlot]);
