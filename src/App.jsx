...
const UPDATE_HISTORY = [
  {
    version: '2.3.1',
    features: [
      '📊 ランキングの順位変動表示を12時間ごとに更新し、変動の大きさを数字（累計差分）で表示するようにしました！'
    ]
  },
  {
    version: '2.3',
    features: [
      '📊 ランキングにて、3時間以内に順位変動があったプレイヤーに矢印（↑・↓）を表示する機能を追加しました！'
    ]
  },
  {
    version: '2.2.1',
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      list.forEach((user, idx) => {
        ...
        let cumulativeDiff = user.cumulativeDiff || 0;
        let lastUpdatedTime = user.lastUpdatedTime || 0;
        let baselineRank = user.baselineRank !== undefined ? user.baselineRank : currentRank;

        if (now - lastUpdatedTime >= twelveHoursMs) {
          const diff = baselineRank - currentRank; // 順位が上がると正(プラス)
          if (diff !== 0) {
            cumulativeDiff += diff;
          }
          ...
          // 12時間未満でもリアルタイムの差分を反映させる場合
          // ここではユーザー要望の「12時間ごとに更新」に合わせて、12時間経過時に一括蓄積、または前回の基準からの差分を累計に加算する仕組みにする
        }

        // 上記を「12時間ごとに蓄積更新する」ロジックに厳密に合わせるため、以下のように実装
        if (!user.baselineRank && user.baselineRank !== 0) {
          baselineRank = currentRank;
          lastUpdatedTime = now;
        }

        if (now - lastUpdatedTime >= twelveHoursMs) {
          const diff = baselineRank - currentRank;
          cumulativeDiff += diff;
          baselineRank = currentRank;
          lastUpdatedTime = now;
        }
        ...
        let trend = 'same';
        if (cumulativeDiff > 0) trend = 'up';
        else if (cumulativeDiff < 0) trend = 'down';

        user.trend = trend;
        user.diffValue = Math.abs(cumulativeDiff);
                  ...
                  return (
                    <div key={idx} className={`ranking-item ${user.userId === userData.userId ? 'highlight-me' : ''}`}>
                      <span style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {user.trend === 'up' && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold', fontSize: '12px' }}>↑{user.diffValue}</span>}
                        {user.trend === 'down' && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold', fontSize: '12px' }}>↓{user.diffValue}</span>}
                      </span>
              ...
              <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '2px dashed #bdc3c7' }}>
                ...
                <div className="ranking-item highlight-me" style={{ margin: '0', borderRadius: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                    ...
                    {myRankingInfo.user.trend === 'up' && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold', fontSize: '12px' }}>↑{myRankingInfo.user.diffValue}</span>}
                    {myRankingInfo.user.trend === 'down' && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold', fontSize: '12px' }}>↓{myRankingInfo.user.diffValue}</span>}
                  </span>
