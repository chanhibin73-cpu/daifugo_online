...
const calcRating = (myRating, allRatings, myRank, finalRankings) => {
  ...
  // 相手プレイヤーごとのレーティング差を計算に加味
  let eloDiff = 0;
  for (let i = 0; i < 4; i++) {
    if (i === myRank) continue;
    const oppRating = allRatings[finalRankings[i]];
    // 格上に勝てばプラス増、格下に負ければマイナス増となる仕組み
    eloDiff += (oppRating - myRating) / 15; 
  }

  let change = Math.round(baseChange + eloDiff);
...
function App() {
  ...
  useEffect(() => {
    if (!userData.username) setShowRegisterModal(true);
  }, [userData.username]);
  ...
  const handleRegister = (e) => {
    ...
    saveUserData({ ...userData, username: inputUsername.trim() });
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    while (currentPlayers.length < 4) {
      // コンピュータのレーティングをランダム(100~900)に生成
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating) => {
    ...
    // CPUレーティングに応じた賢さ判定 (100で10%、900で90%の確率で最適手・温存狙い)
