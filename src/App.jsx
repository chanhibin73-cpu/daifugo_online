...
function App() {
  ...
  useEffect(() => {
    const runAISimulation = async () => {
      const aiIds = ['ai_player_1', 'ai_player_2', 'ai_player_3'];
      const aiData = [];
      ...
      if (aiData.length === 3) {
        ...
        if (matchCount > 0) {
           ...
           for (let m = 0; m < matchCount; m++) {
              ...
              const playersForMatch = [...simPlayers, { userId: 'mob', rating: mobRating }];
    ...
    const initAIPlayers = async () => {
      const aiPlayers = [
        { userId: 'ai_player_1', username: 'AI_Alpha', rating: 1000, maxRating: 1000, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true },
        { userId: 'ai_player_2', username: 'AI_Beta', rating: 1200, maxRating: 1200, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true },
        { userId: 'ai_player_3', username: 'AI_Gamma', rating: 1500, maxRating: 1500, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true }
      ];
      let needsInit = false;
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    if (roomData.type === 'rank') {
      try {
        ...
        snap.forEach(d => {
          const data = d.data();
          if (data.isAIPlayer) aiList.push(data);
        });
      } catch(e) {}
    }

    aiList = aiList.filter(ai => !currentPlayers.some(p => p.userId === ai.userId));
    aiList = shuffle(aiList);

    let cpuCount = 1;
    const baseRating = currentPlayers[0]?.rating || userData.rating;
    
    while (currentPlayers.length < 4) {
      if (roomData.type === 'rank' && aiList.length > 0) {
        const ai = aiList.pop();
    ...
    if (isUltraAI || isEnhancedAI) {
      ...
      if (eights.length > 0) {
         for (let eightMove of eights) {
            ...
            const remNums = new Set(remNonJokers.map(c => c.num));
            if (remNums.size === 1 || (checkSequence(remHand) && remHand.length >= 3)) {
               return eightMove; // ★ 次で上がれるなら無条件で8を出す
            }
         }
      }
