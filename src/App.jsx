...
// レーティング計算ロジック（Ver3.0: 10試合制なら変動量3倍）
...
function App() {
  ...
  const [totalMatches, setTotalMatches] = useState(5); // Ver3.0: 5 or 10
  ...
  useEffect(() => {
    ...
    if (daysPassed >= 5 && userData.rating > 0) {
      const decayDays = daysPassed - 4; // 5日目からカウント
  ...
  // ★ Ver3.0 ランク戦参加ロジック変更 (引数で試合数を受け取る)
  const joinRankMatch = async (matches) => {
    ...
    setIsRankMatch(true); setIsOnlineMode(true); setTotalMatches(matches); setCurrentScreen('waiting_rank'); setRoomTimer(180);
  ...
  const handleSearchFriend = async () => {
    ...
    try {
      ...
      usersSnap.forEach(d => {
        const data = d.data();
        // ★ Ver3.0.1: !data.isAIPlayer の条件を削除し、AIも検索対象に含める
