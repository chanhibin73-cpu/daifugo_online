...
function App() {
  ...
  useEffect(() => {
    if (userData.userId && userData.username) {
      ...
      if (currentScreen === 'game' || currentScreen.startsWith('waiting')) {
      ...
      const interval = setInterval(updateUserStatus, 3 * 60 * 1000); // 3分ごとに更新
      return () => clearInterval(interval);
    }
  }, [userData.userId, userData.username, currentScreen]);
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      // オフライン状態にして送信
      if (userData.userId && userData.username) {
        updateDoc(doc(db, 'users', userData.userId), { status: 'offline', updatedAt: Date.now() }).catch(()=>{});
      }
  ...
  // ★ ランキング内のステータスライトの色とテキストを取得する共通関数
  ...
  return (
    <div className={`app-outer ${userData.uiMode === 'desktop' ? 'desktop-mode' : ''}`} style={containerStyle}>
      <style>{`
        ...
        .desktop-mode .game-board {
          display: flex;
          flex-direction: column;
          height: 100%;
        }
        .desktop-mode .cpu-players {
          flex-direction: row !important;
          justify-content: space-around !important;
          padding: 10px 20px !important;
        }
        .desktop-mode .cpu-card {
          width: 25% !important;
          margin: 0 10px !important;
        }
        .desktop-mode .field-area {
          flex: 1;
          display: flex;
          flex-direction: column;
          justify-content: center;
        }
        .desktop-mode .player-area {
          padding-bottom: 20px !important;
        }
        .desktop-mode .hand-cards {
          justify-content: center !important;
          gap: 5px;
        }
        .desktop-mode .sub-screen,
        .desktop-mode .menu-container {
          max-width: 800px;
          margin: 0 auto;
        }
      `}</style>
                ...
                <div className="ranking-item highlight-me" style={{ margin: '0', borderRadius: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                    ...
                    <span style={{
                      ...
                      height: '8px',
                      borderRadius: '50%',
                      backgroundColor: getStatusColor(myRankingInfo.user),
                      marginLeft: '6px',
                      boxShadow: `0 0 4px ${getStatusColor(myRankingInfo.user)}`
                    }} title={getStatusText(myRankingInfo.user)}></span>
