...
function App() {
    ...
    if (penaltyFlag === 'true') {
      setUserData(prev => {
        ...
        if (updated.username && updated.userId) {
          setDoc(doc(db, 'users', updated.userId), updated, { merge: true }).catch((err)=>{ console.error(err); });
        }
        return updated;
      });
  ...
  const handleDeleteAllData = async () => {
    try { if (userData.userId) await deleteDoc(doc(db, 'users', userData.userId)); } catch (err) { console.error(err); }
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      const thinkTime = isAI ? 3000 : 1000;
