...
function App() {
  ...
  useEffect(() => {
    const rawAiPlayers = [
      ...
      { userId: 'ai_player_81', username: 'AI_asn_v2', rating: 1000, maxRating: 1000, isAIPlayer: true, isSuperAI: true, status: 'online' },
        ...
        if (matchCountSim > 0 && !isRestTime) {
           ...
           for (let mCount = 0; mCount < matchCountSim; mCount++) {
              ...
              playersForMatch.sort((a, b) => {
                ...
                const effA = a.isDoubleGrowthAI ? (2000 + Math.max(0, aMax - 1000) * 4) : (a.isUltraSuperAI ? (3000 + aMax * 3) : (a.isSuperAI ? 2000 : aMax));
                const effB = b.isDoubleGrowthAI ? (2000 + Math.max(0, bMax - 1000) * 4) : (b.isUltraSuperAI ? (3000 + bMax * 3) : (b.isSuperAI ? 2000 : bMax));
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      const now = Date.now();
      const ONE_DAY = 24 * 60 * 60 * 1000; // ★ Ver3.2.2: 更新間隔を1日ごとに変更
      ...
      const processList = (key, baseRankKey, baseRatingKey, updatedAtKey, bestRankKey) => {
        ...
        sortedList.forEach((user, idx) => {
          ...
          // ★ Ver3.2.2: 1日経過でベースのランキング・レートを更新
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    while (currentPlayers.length < 4) {
      if (roomData.type.startsWith('rank') && availableAIList.length > 0 && !isRestTime) {
        ...
        currentPlayers.push({ userId: ai.userId, name: ai.username, rating: aiRating, maxRating: aiMaxRating || aiRating, isCpu: true, isAIPlayer: true, isSuperAI: ai.isSuperAI || false, isUltraSuperAI: ai.isUltraSuperAI || false, isDoubleGrowthAI: ai.isDoubleGrowthAI || false });
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      const move = findBestCpuMove(cpuHand, field, isReversedEffective, cpuThinkRating, hands, turn, isAIPlayer, isSuperAI, isUltraSuperAI, isDoubleGrowthAI);
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, allHands = [], currentTurn = 0, isAIPlayer = false, isSuperAI = false, isUltraSuperAI = false, isDoubleGrowthAI = false) => {
    ...
    // ★ Ver3.2.2 思考力の実質レート計算に isDoubleGrowthAI を追加
    const effectiveRating = isDoubleGrowthAI ? (2000 + Math.max(0, cpuRating - 1000) * 4) : (isUltraSuperAI ? (3000 + cpuRating * 3) : (isSuperAI ? 2000 : (isAIPlayer ? cpuRating * 2 : cpuRating)));
