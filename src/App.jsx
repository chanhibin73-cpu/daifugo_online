...
const UPDATE_HISTORY = [
  {
    version: '2.3.2',
    features: [
      '🤖 新たなAIプレイヤーを1人追加し、合計41人のAIがランク戦に参戦するようになりました（レーティング: 2000）！'
    ]
  },
  {
    version: '2.3.1',
...
// ★ 23:30 ~ 23:59 は AIが活動しない判定
const isAiRestrictedTime = () => {
  const now = new Date();
  return now.getHours() === 23 && now.getMinutes() >= 30;
};

const getPlayerLightColor = (status) => {
...
function App() {
  ...
  useEffect(() => {
    if (userData.userId && userData.username) {
      const status = currentScreen === 'game' ? 'playing' : 'online';
      try {
        setDoc(doc(db, 'users', userData.userId), { status, updatedAt: Date.now() }, { merge: true });
      } catch (e) {}
    }
  ...
  useEffect(() => {
    const runAISimulation = async () => {
      const aiIds = Array.from({ length: 41 }, (_, i) => `ai_player_${i + 1}`);
      const aiData = [];
      ...
      if (aiData.length === 41) {
        ...
        // 23:30 ~ 23:59 は AIの新規マッチングを行わない
        if (matchCount > 0 && !isAiRestrictedTime()) {
           needsUpdate = true;
           for (let m = 0; m < matchCount; m++) {
              const shuffledAI = [...simPlayers].sort(() => Math.random() - 0.5);
              let playersForMatch = [];
              
              // 隠れ更新: AI同士(4人)の対戦を確率で許可する
              if (Math.random() < 0.5) {
    ...
    const initAIPlayers = async () => {
      const aiPlayers = [
        ...
        { userId: 'ai_player_41', username: 'AI_Apex', rating: 2000, maxRating: 2000, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' }
      ];
      let needsInit = false;
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      list.forEach((user, idx) => {
        ...
        // 12時間経過しているか、基準順位が未保存なら現在の順位でリセット更新
        if (!baseRank || (now - baseRankUpdatedAt > TWELVE_HOURS)) {
          baseRank = currentRank;
          baseRankUpdatedAt = now;
          needsUpdate = true;
        }

        // 基準順位からの累計変動数 (値がマイナス＝順位上昇, プラス＝順位下降)
        ...
        // ステータスの補正 (一般ユーザーが10分更新なしの場合はオフラインとみなす)
        if (!user.isAIPlayer) {
          if (now - (user.updatedAt || 0) > 10 * 60 * 1000) {
            user.status = 'offline';
          } else if (!user.status) {
            user.status = 'online';
          }
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    let availableAIList = [];
    if (roomData.type === 'rank' && !isAiRestrictedTime()) {
      try {
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      const move = findBestCpuMove(cpuHand, field, isReversedEffective, cpuRating, hands, turn, isAIPlayer);
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, allHands = [], currentTurn = 0, isAIPlayer = false) => {
                    ...
                    <div key={idx} className={`ranking-item ${user.userId === userData.userId ? 'highlight-me' : ''}`}>
                      <span style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <span style={{
                          ...
                          height: '8px',
                          borderRadius: '50%',
                          backgroundColor: getPlayerLightColor(user.status),
                          marginLeft: '6px',
                          boxShadow: `0 0 4px ${getPlayerLightColor(user.status)}`
                        }} title={getPlayerStatusText(user.status)}></span>
                ...
                <div className="ranking-item highlight-me" style={{ margin: '0', borderRadius: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                    ...
                    <span style={{
                      ...
                      height: '8px',
                      borderRadius: '50%',
                      backgroundColor: getPlayerLightColor(currentScreen === 'game' ? 'playing' : 'online'),
                      marginLeft: '6px',
                      boxShadow: `0 0 4px ${getPlayerLightColor(currentScreen === 'game' ? 'playing' : 'online')}`
                    }} title={getPlayerStatusText(currentScreen === 'game' ? 'playing' : 'online')}></span>
