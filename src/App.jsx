...
const UPDATE_HISTORY = [
  {
    ...
    features: [
      ...
      '🔥 強化コンピュータの調整: プレイヤーレートに比例して強化CPUのレート幅・発生確率（最大10%）が変化し、強化CPUのレーティング下限が1000になりました。'
    ]
  },
  {
    version: '1.1',
    features: [
      '🤖 コンピュータの思考プログラム変更: レーティングに比例してより深い思考（カードの温存など）を行うようになりました。',
...
// プレイヤーのレーティングに比例したCPUのレーティング生成ヘルパー
const generateCpuRating = (playerRating) => {
  ...
  const isEnhanced = Math.random() < enhancedProb;

  if (isEnhanced) {
    const baseEnhanced = 1000 + Math.floor(pRating * 0.2);
...
function App() {
  ...
  const [showDisconnectPenaltyModal, setShowDisconnectPenaltyModal] = useState(false); // ★切断ペナルティポップアップ
  ...
  // ★ 初期ロード時のペナルティチェック（リロードによる切断逃げの検知）
  useEffect(() => {
    const penaltyFlag = localStorage.getItem('daifugo_penalty_flag');
    if (penaltyFlag === 'true') {
      setUserData(prev => {
        const newRating = Math.floor(prev.rating * 0.9); // レーティング10%ダウン
  ...
  // ★ サイトリロード時やタブ閉じ時に、部屋に残っている自分を代行CPUに書き換える処理
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      const cpuTimer = setTimeout(() => {
        ...
        let move = findBestCpuMove(cpuHand, field, isReversedEffective, playerRatings[turn], isSubstitute);
  ...
  // ★ findBestCpuMove に isSubstitute（代行CPU）の引数を追加し、ランダム思考を実装
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false) => {
    ...
    // ★ 途中抜けプレイヤーの代行CPUは、完全にランダムなカードを出す
    if (isSubstitute) {
      if (!currentField) {
        const validSingles = sorted.filter(c => !isForbiddenFinish([c]));
        if (validSingles.length > 0) return [validSingles[Math.floor(Math.random() * validSingles.length)]];
        return [sorted[0]]; // 単体で出せるものがない場合は禁止覚悟で出す
      } else {
        const reqCount = currentField.cards.length;
        if (reqCount === 1) {
          const valid = sorted.filter(c => isValidPlay([c], currentField, isRev) && !isForbiddenFinish([c]));
          ...
          for (let num in groups) {
            if (groups[num].length >= reqCount) {
              ...
              if (isValidPlay(combo, currentField, isRev) && !isForbiddenFinish(combo)) validCombos.push(combo);
            }
          }
          if (validCombos.length > 0) return validCombos[Math.floor(Math.random() * validCombos.length)];
        }
        return null; // 出せるカードがなければパス
      }
    }
  ...
  const handleConfirmReturnHome = async () => {
    ...
    // ★ ホームに戻るボタンからの正常リタイアの場合はフラグを解除（この中でペナルティ計算するため）
    localStorage.removeItem('daifugo_penalty_flag');

    if (isOnlineMode && roomId && roomData) {
      const updatedPlayers = [...(roomData.players || [])];
      if (updatedPlayers[mySlot]) {
        // ★ 自身を「CPU (代行)」に変更し、isSubstitute フラグを付与する
        updatedPlayers[mySlot] = { userId: `cpu_${mySlot}`, name: `CPU (代行)`, rating: userData.rating, isCpu: true, isSubstitute: true };
      }
      await update(ref(rtdb, `rooms/${roomId}`), { players: updatedPlayers });
