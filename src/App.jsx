...
const generateCpuRating = (playerRating) => {
  ...
  // 強化コンピュータの発生確率（プレイヤーレートに比例、上限10%）
  // 例: 500Ptで5%, 1000Pt以上で上限10%
  const enhancedProb = Math.min(0.10, pRating / 10000);
  const isEnhanced = Math.random() < enhancedProb;

  if (isEnhanced) {
    // 強化コンピュータ: 下限1000 + プレイヤーレート比例値 + ランダム幅(0~300)
    const baseEnhanced = 1000 + Math.floor(pRating * 0.2);
    return baseEnhanced + Math.floor(Math.random() * 301);
  } else {
    // 通常コンピュータ: プレイヤーレートを中心に±200の範囲で生成 (下限100)
    const variation = Math.floor(Math.random() * 401) - 200; // -200 ~ +200
