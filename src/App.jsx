...
const UPDATE_HISTORY = [
  {
    ...
    features: [
      ...
      '📊 レーティングシステムの変更: 対戦相手との個別のレーティング差を計算に強く反映するようになりました（格上に勝つと大きく上昇します）。'
    ]
  }
];
...
// レーティング計算ロジック（対戦相手のレーティング差分を個別に反映するシステム）
const calcRating = (myRating, allRatings, myRank, finalRankings) => {
  ...
  let diffBonus = 0;
  // 相手3人それぞれに対して、勝敗とレーティング差分を計算
  for (let i = 0; i < 4; i++) {
    if (i === myRank) continue; // 自分自身はスキップ
    ...
    if (myRank < oppRank) {
      // 相手より上の順位（勝ち）: 相手のレートが高いほどプラスボーナス
      diffBonus += diff * 0.04; 
    } else {
      // 相手より下の順位（負け）: 相手のレートが低いほどマイナスボーナス（diffがマイナスになるため）
      diffBonus += diff * 0.04; 
    }
  }
