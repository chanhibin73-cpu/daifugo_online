...
import { ref, set, get, update, onValue, off, push, remove, onDisconnect } from 'firebase/database';
...
const UPDATE_HISTORY = [
  {
    ...
    features: [
      '🎉 大幅アップデートを行いました！',
      '📡 通信エラー対策: オンライン対戦で通信エラーや切断が起きた場合、自動的にコンピュータが代打ちしてゲームを進行するようになりました。',
      '📊 記録の細分化: プロフィールで「ランク戦」「ルーム戦」「CPU戦」それぞれの戦績（順位確率・連勝数・切断数など）を確認できるようになりました。',
      '⚠️ 切断ペナルティ強化: ランク戦での切断数が多すぎる場合、ペナルティとして一時的な対戦資格の剥奪（1日間）が行われます。',
      '✨ UI/UX改善: より見やすく、どの世代でもプレイしやすいように画面デザインやボタンをモダンに調整しました。'
    ]
  },
  {
    version: '1.9.5',
  ...
  },
  {
    version: '1.9',
    features: [
      '🤖 新機能「AIプレイヤー」を追加しました！自ら思考しランク戦を行い、レーティングランキングにも参加する3人のAIが登場します。'
    ]
  },
  {
    version: '1.8',
...
function App() {
  ...
  const [userData, setUserData] = useState(() => {
    ...
    if (!parsed) {
      parsed = {
        ...
        username: '',
        rating: 500, maxRating: 500,
        wins: [0, 0, 0, 0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0,
        bgColor: '#f4f7f6', bgImage: '', volume: 50, cardDesign: 'mark', cardSize: 'medium',
        lastSeenVersion: '', hasCompleted163Settings: false
      };
    }
    // Ver2.0 データマイグレーション
    if (!parsed.stats) {
      parsed.stats = {
        rank: { wins: parsed.wins || [0,0,0,0], totalGames: parsed.totalGames || 0, currentWinStreak: parsed.currentWinStreak || 0, maxWinStreak: parsed.maxWinStreak || 0, disconnects: 0 },
    ...
    if (parsed.banUntil === undefined) parsed.banUntil = null;
    return parsed;
  });
  ...
  useEffect(() => {
    const runAISimulation = async () => {
      const aiIds = Array.from({length: 12}, (_, i) => `ai_player_${i+1}`);
      const aiData = [];
      ...
      if (aiData.length === 12) {
        ...
        if (matchCount > 0 || needsUpdate) {
            for (const ai of simPlayers) {
               ...
               try { await setDoc(doc(db, 'users', ai.userId), ai, { merge: true }); } catch(e){}
            }
        }
      }
    };
    ...
    const initAIPlayers = async () => {
      ...
      const ratings = [1000, 1200, 1500, 900, 1100, 1300, 1400, 1600, 1050, 1250, 1450, 1700];
      const aiPlayers = names.map((name, i) => ({
        userId: `ai_player_${i+1}`, username: `AI_${name}`, rating: ratings[i], maxRating: ratings[i],
        wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online'
      }));
      ...
      if (!needsInit) runAISimulation();
    };
    initAIPlayers();
  }, []);

  // ★ Ver2.0 ペナルティチェック（BAN含む）
  useEffect(() => {
    ...
    if (penaltyFlag === 'true') {
      setUserData(prev => {
        ...
        let newBanUntil = prev.banUntil || null;
        let isBannedNow = false;
        if (newStats.rank.disconnects >= 3 && newStats.rank.disconnects % 3 === 0) {
           newBanUntil = Date.now() + 24 * 60 * 60 * 1000; // 24時間BAN
           isBannedNow = true;
        }

        const newRating = Math.floor(prev.rating * 0.9);
        const updated = { ...prev, rating: newRating, currentWinStreak: 0, stats: newStats, banUntil: newBanUntil };
        ...
        if (isBannedNow) {
          setDisconnectMessage('悪質な切断が検知されたため、1日間（24時間）のランク戦参加資格を剥奪しました。');
        } else {
          setDisconnectMessage(`前回のランク戦で切断が検知されました。\nペナルティとしてレーティングが 10% 減少しました。\n（切断数: ${newStats.rank.disconnects} 回）`);
        }
  ...
  useEffect(() => { if (!userData.username) setShowRegisterModal(true); }, [userData.username]);
  ...
  // ★ Ver2.0 プレゼンス設定 (オンライン対戦時の通信監視用)
  useEffect(() => {
    if (isOnlineMode && roomId && userData.userId) {
      const myConnRef = ref(rtdb, `rooms/${roomId}/connections/${userData.userId}`);
      set(myConnRef, true);
      onDisconnect(myConnRef).remove();
      return () => {
        remove(myConnRef);
        onDisconnect(myConnRef).cancel();
      };
    }
  }, [isOnlineMode, roomId, userData.userId]);

  // ★ Ver2.0 ホストによる通信エラー監視 (代打ち機能)
  useEffect(() => {
    if (isOnlineMode && isHost && roomId && currentScreen === 'game') {
      const connsRef = ref(rtdb, `rooms/${roomId}/connections`);
      const unsub = onValue(connsRef, (snap) => {
        const conns = snap.val() || {};
        if (roomData && roomData.players) {
          let changed = false;
          const newPlayers = [...roomData.players];
          newPlayers.forEach((p, idx) => {
            if (!p.isCpu && !conns[p.userId]) {
              // プレイヤーの通信が途切れた -> 代行CPUに切り替え
              newPlayers[idx] = { ...p, isCpu: true, isSubstitute: true, name: 'CPU (代行)' };
              changed = true;
            }
          });
          if (changed) {
            update(ref(rtdb, `rooms/${roomId}`), { players: newPlayers });
          }
        }
      });
      return () => off(connsRef);
    }
  }, [isOnlineMode, isHost, roomId, currentScreen, roomData]);
  ...
  const joinRankMatch = async () => {
    ...
    if (userData.banUntil && Date.now() < userData.banUntil) {
      const remainHours = Math.ceil((userData.banUntil - Date.now()) / (1000 * 60 * 60));
      alert(`切断ペナルティにより、ランク戦への参加が制限されています。\n解除まで残り約 ${remainHours} 時間`);
      return;
    }
  ...
  timeoutRef.current = async () => {
    ...
    if (isOnlineMode && roomId && roomData) {
      ...
      if (isRankMatch) {
        newStats.rank.disconnects = (newStats.rank.disconnects || 0) + 1;
        if (newStats.rank.disconnects >= 3 && newStats.rank.disconnects % 3 === 0) {
           newBanUntil = Date.now() + 24 * 60 * 60 * 1000; 
        }
        penaltyRating = Math.max(0, userData.rating - 15);
        saveUserData({ ...userData, rating: penaltyRating, totalGames: userData.totalGames + 1, currentWinStreak: 0, stats: newStats, banUntil: newBanUntil });
      }
      setRoomId(null); setRoomData(null); setMySlot(0);
    }
    
    localStorage.removeItem('daifugo_penalty_flag');
    if (newBanUntil && newBanUntil > Date.now()) {
        setDisconnectMessage('悪質な放置・切断が検知されたため、1日間（24時間）のランク戦参加資格を剥奪しました。');
    } else {
        setDisconnectMessage(`持ち時間超過により退出しました。\nペナルティとしてレーティングが 10% 減少しました。\n（切断数: ${newStats.rank?.disconnects || 0} 回）`);
    }
    setCurrentScreen('menu');
    setShowDisconnectPenaltyModal(true);
  };
  ...
  const finish5MatchGameSet = (finalScores) => {
    ...
    if (myFinalRank === 0) {
      newStats[mode].currentWinStreak += 1;
    ...
    // 全体互換用の変数
    const newWins = [0,1,2,3].map(i => newStats.rank.wins[i] + newStats.room.wins[i] + newStats.cpu.wins[i]);
    const newTotalGames = newStats.rank.totalGames + newStats.room.totalGames + newStats.cpu.totalGames;
    const newCurrentWinStreak = newStats[mode].currentWinStreak;
    const newMaxWinStreak = Math.max(userData.maxWinStreak || 0, newStats[mode].currentWinStreak);
    ...
    const updatedUser = {
      ...userData, 
      rating: newRating, maxRating: Math.max(userData.maxRating, newRating),
      wins: newWins, totalGames: newTotalGames, currentWinStreak: newCurrentWinStreak, maxWinStreak: newMaxWinStreak,
      stats: newStats
    };
  ...
  const calcRateObj = (count, total) => { return total === 0 ? '0%' : `${((count / total) * 100).toFixed(1)}%`; };

  // ★ Ver2.0 UI/UX のための共通ボタンスタイルクラス名
  const modernBtnStyle = "action-btn modern-btn shadow-sm";
  ...
  return (
    <div className="app-outer" style={containerStyle}>
      <style>{`
        ...
        .modern-btn { border-radius: 8px; font-weight: bold; transition: all 0.2s; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        .modern-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 8px rgba(0,0,0,0.15); }
        .tab-btn { flex: 1; padding: 10px; font-weight: bold; border-radius: 8px; border: none; cursor: pointer; transition: 0.2s; }
        .tab-btn.active { background-color: #3498db; color: white; box-shadow: 0 2px 4px rgba(52, 152, 219, 0.4); }
        .tab-btn.inactive { background-color: #ecf0f1; color: #7f8c8d; }
      `}</style>
      <div className="app-container" style={{ borderRadius: '16px', boxShadow: '0 10px 20px rgba(0,0,0,0.2)' }}>
          ...
          <div className="modal-overlay">
            <div className="modal-content" style={{ borderRadius: '16px' }}>
              ...
              <p style={{ fontSize: '14px', lineHeight: '1.5' }}>
                ...
                <span style={{ fontSize: '12px', color: '#7f8c8d' }}>(後から「設定」で変更可能です)</span>
              </p>
              
              <div className="setting-item" style={{ textAlign: 'left', marginTop: '15px' }}>
                <label style={{ display: 'block', fontSize: '15px', fontWeight: 'bold' }}>トランプのデザイン</label>
                <select style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '8px', fontSize: '14px' }} value={userData.cardDesign} onChange={(e) => saveUserData({ ...userData, cardDesign: e.target.value })}>
              ...
              <div className="setting-item" style={{ textAlign: 'left', marginTop: '15px' }}>
                <label style={{ display: 'block', fontSize: '15px', fontWeight: 'bold' }}>カードのサイズ</label>
                <select style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '8px', fontSize: '14px' }} value={userData.cardSize} onChange={(e) => saveUserData({ ...userData, cardSize: e.target.value })}>
              ...
              <button className={modernBtnStyle} onClick={() => {
                setShowInitialSettingsModal(false);
                saveUserData({ ...userData, hasCompleted163Settings: true });
              }} style={{ marginTop: '20px', padding: '12px' }}>決定して進む</button>
            </div>
          </div>
        )}
        ...
        {showDisconnectPenaltyModal && (
          <div className="modal-overlay">
            <div className="modal-content" style={{ borderRadius: '16px' }}>
              ...
              <button className={`${modernBtnStyle} warning-btn`} onClick={() => setShowDisconnectPenaltyModal(false)} style={{ marginTop: '15px', padding: '12px' }}>確認しました</button>
            </div>
          </div>
        )}
        ...
        {showUpdateModal && (
          <div className="modal-overlay">
            <div className="modal-content" style={{ borderRadius: '16px' }}>
              ...
              <div className="rules-scroll-area" style={{ textAlign: 'left', fontSize: '13px', lineHeight: '1.6', maxHeight: '350px' }}>
                {UPDATE_HISTORY.map((update, idx) => (
                  <div key={idx} style={{ marginBottom: '15px' }}>
                    <p style={{ margin: '0 0 5px 0', fontWeight: 'bold', borderBottom: '1px solid #ddd', paddingBottom: '3px', color: '#34495e' }}>
                      Ver {update.version}
                    </p>
              ...
              <button className={modernBtnStyle} onClick={handleCloseUpdate} style={{ marginTop: '15px', padding: '12px' }}>確認しました</button>
            </div>
          </div>
        )}
          ...
          <div className="modal-overlay">
            <div className="modal-content" style={{ borderRadius: '16px' }}>
              ...
              <form onSubmit={handleRegister}>
                <input type="text" style={{ fontSize: '1.2rem', padding: '12px 16px', width: '90%', margin: '15px 0', borderRadius: '8px', border: '2px solid #3498db', outline: 'none' }} value={inputUsername} onChange={(e) => setInputUsername(e.target.value)} placeholder="ユーザーネーム" maxLength={10} required />
                <button type="submit" className={modernBtnStyle} style={{ padding: '12px' }}>登録する</button>
              </form>
            </div>
          </div>
        )}
        ...
        {showRecommendModal && (
          <div className="modal-overlay">
            <div className="modal-content recommend-modal" style={{ borderRadius: '16px' }}>
              ...
              <a href="https://hibimaruwebappscollection.vercel.app/index.html" target="_blank" rel="noopener noreferrer" className="recommend-link-btn" style={{ borderRadius: '8px' }}>サイトを見る</a>
              <div><button className={`${modernBtnStyle} next-modal-btn`} onClick={handleCloseRecommend}>次へ（ルールを確認）</button></div>
            </div>
          </div>
        )}
        ...
        {showRulesModal && (
          <div className="modal-overlay">
            <div className="modal-content rules-modal-content" style={{ borderRadius: '16px' }}>
              <h3>📜 導入されているルール 📜</h3>
              <div className="rules-scroll-area" style={{ fontSize: '13px', lineHeight: '1.6' }}>
                <ul>
              ...
              <button className={modernBtnStyle} onClick={() => setShowRulesModal(false)} style={{ padding: '12px' }}>ゲームを始める</button>
            </div>
          </div>
        )}
        ...
        {showConfirmHomeModal && (
          <div className="modal-overlay">
            <div className="modal-content" style={{ borderRadius: '16px' }}>
              <h3>ホームへ戻りますか？</h3>
              <p style={{ fontSize: '14px' }}>{isOnlineMode ? '※対戦中に退出すると代行CPUに切り替わり、ペナルティとしてレーティングが減少します。' : '対戦を終了してメニューへ戻ります。'}</p>
              <div className="modal-actions" style={{ marginTop: '20px' }}>
                <button className={`${modernBtnStyle} warning-btn`} onClick={handleConfirmReturnHome}>はい（退出）</button>
                <button className={`${modernBtnStyle} back-btn`} onClick={() => setShowConfirmHomeModal(false)} style={{ backgroundColor: '#95a5a6' }}>キャンセル</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showDeleteDataModal && (
          <div className="modal-overlay">
            <div className="modal-content" style={{ borderRadius: '16px' }}>
              <h3>⚠️ データの全削除確認</h3>
              <p style={{ fontSize: '14px' }}>オンラインランキング上のデータを含め、すべてのプレイデータが完全に削除されます。<br />本当に削除しますか？</p>
              <div className="modal-actions" style={{ marginTop: '20px' }}>
                <button className={`${modernBtnStyle} warning-btn`} onClick={handleDeleteAllData}>全データを削除する</button>
                <button className={`${modernBtnStyle} back-btn`} onClick={() => setShowDeleteDataModal(false)} style={{ backgroundColor: '#95a5a6' }}>キャンセル</button>
              </div>
            </div>
          </div>
        )}
        ...
        {currentScreen === 'menu' && (
          <div className="menu-container">
            <h1 className="title" style={{ fontWeight: '800', letterSpacing: '2px' }}>
              大富豪オンライン
              <div style={{ fontSize: '14px', color: '#7f8c8d', marginTop: '6px', fontWeight: 'normal' }}>Ver.{CURRENT_VERSION}</div>
            </h1>
            <p className="user-badge" style={{ fontSize: '16px', marginBottom: '30px' }}>ようこそ、<strong style={{ color: '#2c3e50' }}>{userData.username || 'ゲスト'}</strong> さん</p>

            <button className={`${modernBtnStyle} menu-btn`} onClick={() => setCurrentScreen('online_select')} style={{ padding: '14px' }}>オンライン対戦</button>
            <button className={`${modernBtnStyle} menu-btn`} onClick={() => { setMySlot(0); startNewGameSet(['あなた', 'CPU 1', 'CPU 2', 'CPU 3'], false, false); }} style={{ padding: '14px' }}>コンピュータ対戦</button>
            <button className={`${modernBtnStyle} menu-btn`} onClick={() => setCurrentScreen('ranking')} style={{ padding: '14px' }}>全プレイヤーレーティング</button>
            <button className={`${modernBtnStyle} menu-btn`} onClick={() => setCurrentScreen('profile')} style={{ padding: '14px' }}>プロフィール・戦績</button>
            <button className={`${modernBtnStyle} menu-btn`} onClick={() => setCurrentScreen('settings')} style={{ padding: '14px' }}>設定・ルール</button>
          </div>
        )}
        ...
        {currentScreen === 'online_select' && (
          <div className="sub-screen">
            <h2>オンライン対戦</h2>
            <div className="menu-container">
              <button className={`${modernBtnStyle} menu-btn mode-btn`} onClick={joinRankMatch} style={{ padding: '16px' }}>
                ランク戦
                <span className="btn-subtext" style={{ fontSize: '13px' }}>レーティング競い合い / 全国ランキング</span>
              </button>
              <button className={`${modernBtnStyle} menu-btn mode-btn`} onClick={() => setCurrentScreen('room_select')} style={{ padding: '16px' }}>
                ルーム戦
                <span className="btn-subtext" style={{ fontSize: '13px' }}>4桁の合い言葉で合流</span>
              </button>
            </div>
            <button className={`${modernBtnStyle} back-btn`} onClick={() => setCurrentScreen('menu')} style={{ backgroundColor: '#95a5a6' }}>戻る</button>
          </div>
        )}
        ...
        {currentScreen === 'room_select' && (
          <div className="sub-screen">
            <h2>ルーム戦</h2>
            <button className={`${modernBtnStyle} menu-btn`} onClick={createRoom} style={{ padding: '14px' }}>部屋を作成する</button>
            <div className="room-input-box" style={{ marginTop: '20px' }}>
              <input type="number" placeholder="4桁のルーム番号" value={inputRoomCode} onChange={(e) => setInputRoomCode(e.target.value.slice(0, 4))} style={{ borderRadius: '8px', border: '2px solid #ccc', padding: '12px' }} />
              <button className={modernBtnStyle} onClick={joinRoomByCode} style={{ padding: '12px' }}>部屋に入る</button>
            </div>
            <button className={`${modernBtnStyle} back-btn`} onClick={() => setCurrentScreen('online_select')} style={{ backgroundColor: '#95a5a6' }}>戻る</button>
          </div>
        )}
        ...
        {currentScreen === 'waiting_rank' && (
          <div className="sub-screen">
            <h2>ランク戦 待機ルーム</h2>
            <p className="timer-text" style={{ fontSize: '18px', fontWeight: 'bold', color: '#e74c3c' }}>試合開始まで: {Math.floor(roomTimer / 60)}分 {roomTimer % 60}秒</p>
            <p className="info-text">※3分経過時に不足分をCPUで補填して開始します</p>

            <div className="player-list-box" style={{ borderRadius: '12px', padding: '15px' }}>
              <h4>参加プレイヤー ({roomData?.players ? roomData.players.length : 1} / 4人)</h4>
              {roomData?.players?.map((p, idx) => (
                <div key={idx} className="player-item" style={{ fontSize: '16px', padding: '8px 0' }}>👤 {p.name} ({p.rating} Pt)</div>
              ))}
            </div>

            <button className={modernBtnStyle} onClick={startOnlineGameWithCpu} style={{ padding: '14px', width: '100%', marginBottom: '15px' }}>今すぐ対戦開始 (CPU補充)</button>
            <button className={`${modernBtnStyle} back-btn`} onClick={async () => { await handleLeaveRoom(); setCurrentScreen('online_select'); }} style={{ backgroundColor: '#95a5a6' }}>退出する</button>
          </div>
        )}
        ...
        {currentScreen === 'waiting_room' && (
          <div className="sub-screen">
            ...
            <div className="player-list-box" style={{ borderRadius: '12px', padding: '15px' }}>
              ...
              {roomData?.players?.map((p, idx) => (
                <div key={idx} className="player-item" style={{ fontSize: '16px', padding: '8px 0' }}>👤 {p.name}</div>
              ))}
            </div>

            {isHost ? (
              <button className={modernBtnStyle} onClick={startOnlineGameWithCpu} style={{ padding: '14px', width: '100%', marginBottom: '15px' }}>試合スタート</button>
            ) : (
              <p className="info-text" style={{ fontWeight: 'bold', color: '#f39c12' }}>ホストの開始を待っています...</p>
            )}
        ...
        {currentScreen === 'game' && (
          <div className="game-board">
            <div className="game-header">
              <span style={{ fontWeight: 'bold' }}>{matchCount} / 5 試合</span>
              <span style={{ fontWeight: 'bold' }}>Pt: {scores[mySlot]}</span>
              ...
              <button className="home-btn-small" onClick={() => setShowConfirmHomeModal(true)} style={{ borderRadius: '8px', border: 'none', padding: '6px 10px', fontWeight: 'bold' }}>🏠 ホームへ</button>
            </div>

            <div className="cpu-players">
              {[0, 1, 2, 3].filter(idx => idx !== mySlot).map(idx => (
                <div key={idx} className={`cpu-card ${turn === idx && !exchangePhase ? 'active-turn' : ''}`} style={{ borderRadius: '12px', boxShadow: turn === idx ? '0 0 10px rgba(52,152,219,0.5)' : 'none' }}>
                  <div className="cpu-name" style={{ fontWeight: 'bold' }}>{players[idx]}</div>
                  <div className="cpu-cards-count" style={{ fontSize: '14px' }}>🂠 {hands[idx]?.length || 0}枚</div>
                  <div className="cpu-score">Pt: {scores[idx]}</div>
                  <div className="cpu-score" style={{fontSize: "10px", color:"#7f8c8d", marginTop: "2px"}}>R: {playerRatings[idx]}</div>
                </div>
              ))}
            </div>

            <div className="log-banner" style={{ borderRadius: '8px', fontSize: '14px', padding: '10px' }}>{message}</div>

            <div className="field-area" style={exchangePhase ? { backgroundColor: 'transparent' } : {}}>
              {exchangePhase ? (
                <div style={{ textAlign: 'center', width: '100%', padding: '20px', background: 'rgba(255,255,255,0.95)', borderRadius: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.15)' }}>
                  <h4 style={{ color: '#e74c3c', margin: '0 0 15px 0', fontSize: '18px' }}>🔄 カード交換フェーズ</h4>
                  {exchangeCards[mySlot] ? (
                    <p style={{ fontSize: '14px', color: '#555', fontWeight: 'bold' }}>他のプレイヤーの準備を待っています...</p>
                  ) : (
                    <div>
                      {prevRanks.indexOf(mySlot) === 0 || prevRanks.indexOf(mySlot) === 1 ? (
                        <>
                          <p style={{ fontSize: '13px', margin: '10px 0', fontWeight: 'bold' }}>いらないカードを {prevRanks.indexOf(mySlot) === 0 ? 2 : 1}枚 選択してください</p>
                          <button 
                            className={modernBtnStyle}
                        ...
                        <>
                          <p style={{ fontSize: '13px', margin: '10px 0', fontWeight: 'bold' }}>あなたの最強カードが自動的に {prevRanks.indexOf(mySlot) === 3 ? 2 : 1}枚 渡されます。</p>
                          <button className={modernBtnStyle} style={{ padding: '10px 20px', fontSize: '14px' }} onClick={handleExchangeSubmit}>確認して渡す</button>
                        </>
                      )}
                    </div>
                  )}
                ...
                <>
                  <div className="field-title" style={{ fontWeight: 'bold' }}>場</div>
                  <div className="field-cards">
                    {field ? (
                      ...
                      <div className="empty-field" style={{ fontSize: '14px' }}>カードはありません</div>
                    )}
                  </div>
                </>
              )}
            ...
            <div className="player-area">
              <div className="player-info">
                <span style={{ fontWeight: 'bold', fontSize: '14px' }}>あなたの手札 ({hands[mySlot]?.length || 0}枚)</span>
                {turn === mySlot && !exchangePhase && <span className="your-turn-badge" style={{ fontSize: '12px', padding: '4px 8px' }}>あなたの番です！</span>}
              </div>

              {isRankMatch && !exchangePhase && (
                <div style={{ textAlign: 'left', fontSize: '14px', fontWeight: 'bold', marginBottom: '8px', color: turn === mySlot && turnTimer <= 10 ? '#e74c3c' : '#2c3e50' }}>
              ...
              {!exchangePhase && (
                <div className="controls" style={{ marginTop: '15px' }}>
                  <button
                    className={`${modernBtnStyle} play-btn`}
                  ...
                  <button
                    className={`${modernBtnStyle} pass-btn`}
                    style={{ padding: '14px 24px', fontSize: '16px', backgroundColor: '#95a5a6' }}
            ...
            {matchResultModal && (
              <div className="modal-overlay">
                <div className="modal-content" style={{ borderRadius: '16px' }}>
                  ...
                  <div className="result-list" style={{ marginTop: '15px' }}>
                    {matchResultModal.rankings.map((pIdx, rank) => (
                      <div key={rank} className={`result-item ${pIdx === mySlot ? 'highlight-me' : ''}`} style={{ fontSize: '16px', padding: '10px' }}>
                        <span style={{ fontWeight: 'bold' }}>{rank + 1}位: {players[pIdx]}</span>
                  ...
                  {isHost ? (
                    <button className={modernBtnStyle} onClick={() => {
                      ...
                      }} style={{ padding: '12px', marginTop: '20px' }}>次の試合へ</button>
                  ) : (
                    <p style={{fontSize: '14px', fontWeight: 'bold', color: '#f39c12', marginTop: '20px'}}>ホストが次の試合を開始するのをお待ちください...</p>
                  )}
                </div>
              </div>
            )}
                ...
                <div className="modal-content" style={{ borderRadius: '16px' }}>
                  ...
                  <div className="result-list" style={{ marginTop: '15px' }}>
                    {gameResultModal.finalRankings.map((pIdx, rank) => (
                      <div key={rank} className={`result-item ${pIdx === mySlot ? 'highlight-me' : ''}`} style={{ fontSize: '16px', padding: '10px' }}>
                        <span style={{ fontWeight: 'bold' }}>{rank + 1}位: {players[pIdx]}</span>
                        <span style={{ fontWeight: 'bold', color: '#e74c3c' }}>合計 {gameResultModal.scores[pIdx]} Pt</span>
                      </div>
                    ))}
                  </div>

                  {isRankMatch && (
                    <div className="rating-change-box" style={{ borderRadius: '8px', padding: '15px', marginTop: '20px' }}>
                      <p style={{ fontSize: '15px', fontWeight: 'bold' }}>レート変動: <span style={{ color: gameResultModal.ratingChange >= 0 ? '#2ecc71' : '#e74c3c' }}>{gameResultModal.ratingChange >= 0 ? `+${gameResultModal.ratingChange}` : gameResultModal.ratingChange} Pt</span></p>
                      <p className="new-rating" style={{ fontSize: '18px' }}>新レーティング: {gameResultModal.newRating} Pt</p>
                    </div>
                  )}

                  <button className={modernBtnStyle} onClick={handleFinishGameAndLeave} style={{ padding: '12px', marginTop: '20px' }}>
                    メニューへ戻る
                  </button>
                </div>
              </div>
        ...
        {currentScreen === 'ranking' && (
          <div className="sub-screen">
            ...
            <button className={modernBtnStyle} style={{ marginBottom: '15px', padding: '10px 20px', fontSize: '14px' }} onClick={fetchRankings} disabled={rankingLoading}>
              {rankingLoading ? '読み込み中...' : '🔄 最新情報に更新'}
            </button>

            {rankingError && (<p className="error-text" style={{ color: '#e74c3c', padding: '10px', background: 'rgba(231,76,60,0.1)', borderRadius: '8px', fontWeight: 'bold' }}>{rankingError}</p>)}

            <div className="ranking-list" style={{ borderRadius: '12px' }}>
              {rankingLoading ? (
                <div className="ranking-item" style={{ padding: '15px' }}>プレイデータ読み込み中...</div>
                ...
                globalRankings.map((user, idx) => {
                  ...
                  return (
                    <div key={idx} className={`ranking-item ${user.userId === userData.userId ? 'highlight-me' : ''}`} style={{ padding: '12px 15px', fontSize: '15px' }}>
                      <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                        ...
                        {user.isAIPlayer && (
                          <span style={{
                            ...
                            marginLeft: '8px',
                            boxShadow: `0 0 5px ${getAILightColor(user.status)}`
                          }} title={getAIStatusText(user.status)}></span>
                        )}
                      </span>
                      <span style={{ fontWeight: 'bold', color: '#2c3e50' }}>{user.rating} Pt</span>
                    </div>
                  );
                })
              ) : (
                <div className="ranking-item" style={{ padding: '15px' }}>登録されているプレイヤーがいません</div>
              )}
            </div>
            <button className={`${modernBtnStyle} back-btn`} onClick={() => setCurrentScreen('menu')} style={{ backgroundColor: '#95a5a6' }}>戻る</button>
          </div>
        ...
        {/* ★ Ver2.0: プロフィール画面の大幅改善（タブ式詳細戦績） */}
        {currentScreen === 'profile' && (
          <div className="sub-screen profile-screen">
            <h2>プロフィール・戦績</h2>
            
            <div className="profile-card" style={{ padding: '20px', borderRadius: '16px' }}>
              <div className="profile-row" style={{ fontSize: '16px' }}><span className="label">ユーザーネーム:</span><span className="val" style={{ fontWeight: 'bold' }}>{userData.username}</span></div>
              <div className="profile-row" style={{ fontSize: '16px' }}><span className="label">現在のレーティング:</span><span className="val highlight" style={{ fontSize: '20px' }}>{userData.rating} Pt</span></div>
              <div className="profile-row" style={{ fontSize: '16px' }}><span className="label">最高レーティング:</span><span className="val" style={{ fontWeight: 'bold' }}>{userData.maxRating} Pt</span></div>
            </div>

            <h3 style={{ margin: '20px 0 10px 0', fontSize: '18px', color: '#2c3e50' }}>📊 モード別 戦績記録</h3>
            
            <div style={{ display: 'flex', gap: '8px', marginBottom: '15px' }}>
              <button className={`tab-btn ${profileTab === 'rank' ? 'active' : 'inactive'}`} onClick={() => setProfileTab('rank')}>ランク戦</button>
              <button className={`tab-btn ${profileTab === 'room' ? 'active' : 'inactive'}`} onClick={() => setProfileTab('room')}>ルーム戦</button>
              <button className={`tab-btn ${profileTab === 'cpu' ? 'active' : 'inactive'}`} onClick={() => setProfileTab('cpu')}>CPU戦</button>
            </div>

            <div className="profile-card" style={{ padding: '20px', borderRadius: '16px', backgroundColor: '#fff', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
              {(() => {
                const stats = userData.stats?.[profileTab] || { wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, disconnects: 0 };
                return (
                  <>
                    <h4 style={{ margin: '0 0 15px 0', textAlign: 'center', color: '#34495e' }}>各順位 獲得確率 (総プレイ: {stats.totalGames}回)</h4>
                    <div className="stats-grid" style={{ marginBottom: '20px' }}>
                      <div style={{ padding: '10px', fontWeight: 'bold' }}>1位: <br/><span style={{ color: '#e74c3c', fontSize: '18px' }}>{calcRateObj(stats.wins[0], stats.totalGames)}</span></div>
                      <div style={{ padding: '10px', fontWeight: 'bold' }}>2位: <br/><span style={{ color: '#f39c12', fontSize: '18px' }}>{calcRateObj(stats.wins[1], stats.totalGames)}</span></div>
                      <div style={{ padding: '10px', fontWeight: 'bold' }}>3位: <br/><span style={{ color: '#2ecc71', fontSize: '18px' }}>{calcRateObj(stats.wins[2], stats.totalGames)}</span></div>
                      <div style={{ padding: '10px', fontWeight: 'bold' }}>4位: <br/><span style={{ color: '#3498db', fontSize: '18px' }}>{calcRateObj(stats.wins[3], stats.totalGames)}</span></div>
                    </div>
                    <hr style={{ borderTop: '1px dashed #ecf0f1', margin: '15px 0' }} />
                    <div className="profile-row" style={{ fontSize: '15px' }}><span className="label">現在の連勝数:</span><span className="val" style={{ fontWeight: 'bold' }}>{stats.currentWinStreak} 連勝</span></div>
                    <div className="profile-row" style={{ fontSize: '15px' }}><span className="label">最高連勝数:</span><span className="val" style={{ fontWeight: 'bold' }}>{stats.maxWinStreak} 連勝</span></div>
                    <div className="profile-row" style={{ fontSize: '15px', color: stats.disconnects > 0 ? '#e74c3c' : '#7f8c8d' }}>
                      <span className="label">切断数:</span><span className="val" style={{ fontWeight: 'bold' }}>{stats.disconnects || 0} 回</span>
                    </div>
                  </>
                );
              })()}
            ...
            <button className={`${modernBtnStyle} back-btn`} onClick={() => setCurrentScreen('menu')} style={{ backgroundColor: '#95a5a6', marginTop: '20px' }}>戻る</button>
          </div>
        )}
        ...
        {currentScreen === 'settings' && (
          <div className="sub-screen settings-screen">
            ...
            <div className="setting-item" style={{ padding: '15px', borderRadius: '12px' }}>
              <label style={{ fontWeight: 'bold' }}>ユーザーネーム変更</label>
              <input type="text" style={{ fontSize: '1.1rem', padding: '10px 12px', width: '100%', boxSizing: 'border-box', borderRadius: '8px', border: '1px solid #ccc', marginTop: '5px' }} value={userData.username} onChange={(e) => saveUserData({ ...userData, username: e.target.value })} maxLength={10} />
            </div>

            <div className="setting-item" style={{ padding: '15px', borderRadius: '12px' }}>
              <label style={{ fontWeight: 'bold' }}>背景色変更</label>
              <input type="color" value={userData.bgColor} onChange={(e) => saveUserData({ ...userData, bgColor: e.target.value, bgImage: '' })} style={{ marginLeft: '15px', cursor: 'pointer' }} />
            </div>

            <div className="setting-item" style={{ padding: '15px', borderRadius: '12px' }}>
              <label style={{ fontWeight: 'bold' }}>トランプのデザイン</label>
              <select style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '8px', fontSize: '14px' }} value={userData.cardDesign} onChange={(e) => saveUserData({ ...userData, cardDesign: e.target.value })}>
                <option value="mark">数字とマーク</option>
                <option value="number">数字のみ</option>
              </select>
            ...
            <div className="setting-item" style={{ padding: '15px', borderRadius: '12px' }}>
              <label style={{ fontWeight: 'bold' }}>カードのサイズ</label>
              <select style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '8px', fontSize: '14px' }} value={userData.cardSize} onChange={(e) => saveUserData({ ...userData, cardSize: e.target.value })}>
                <option value="x-small">極小</option>
                <option value="small">小</option>
                <option value="medium">中</option>
            ...
            <div className="setting-rules-box" style={{ borderRadius: '12px', padding: '15px' }}>
              <h4 style={{ fontSize: '16px', marginBottom: '10px' }}>📜 採用ルール一覧</h4>
              <ul className="rules-mini-list" style={{ fontSize: '13px', lineHeight: '1.6' }}>
                <li><strong>🔒 スート縛り:</strong> 同じマーク(複数枚も可)が3連続で出ると、場が流れるまで同マークしか出せません。</li>
            ...
            <div className="setting-rules-box" style={{ marginTop: '15px', borderRadius: '12px', padding: '15px' }}>
              <h4 style={{ fontSize: '16px', marginBottom: '5px' }}>💻 推奨利用環境</h4>
              <p style={{ fontSize: '12px', margin: '5px 0', color: '#555', lineHeight: '1.5' }}>
                より快適にプレイしていただくため、最新版の Safari, Chrome, Edge などのブラウザのご利用を推奨します。
              </p>
            </div>

            <div style={{ marginTop: '20px', textAlign: 'center' }}>
              <button className={modernBtnStyle} style={{ backgroundColor: '#3498db', width: '100%', padding: '14px' }} onClick={() => setShowUpdateModal(true)}>
                📢 更新情報 (リリースノート) を確認
              </button>
            </div>

            <div style={{ marginTop: '15px', textAlign: 'center' }}>
              <button className={`${modernBtnStyle} warning-btn`} style={{ backgroundColor: '#e74c3c', width: '100%', padding: '14px' }} onClick={() => setShowDeleteDataModal(true)}>
                🗑 全データを削除して初期化
              </button>
            </div>
