...
const UPDATE_HISTORY = [
  {
    version: '1.5',
    features: [
      '⏱ 持ち時間システムの導入: ランク戦において、1手ごとの持ち時間（30秒）を追加しました。時間切れで自動的にパスとなります。'
    ]
  },
  {
    version: '1.4.1',
  ...
  },
  {
    version: '1.3.1',
    features: [
      '🐛 バグ修正: 8を含んだカードで禁止上がりをした際に、禁止上がりの表示が出ずゲームが進行しなくなるバグを修正しました。'
    ]
  },
  {
    version: '1.3',
    features: [
      ...
      '🤖 新機能: プレイヤーレート800以上で、高度な状況判断を行う「AIコンピュータ」が低確率で出現するようになりました。',
  ...
  },
  {
    ...
    features: [
      ...
      '🔥 強化コンピュータの調整: プレイヤーレートに比例して強化CPUの発生確率が変化し、下限が1000になりました。'
    ]
  },
  {
    version: '1.1',
...
const generateCpuRating = (playerRating) => {
  ...
  let aiProb = 0;
  if (pRating >= 1200) {
    aiProb = 0.05 + Math.min(0.05, (pRating - 1200) / 10000); 
  }

  let enhancedProb = Math.min(0.20, pRating / 5000); 
  if (pRating >= 1000) {
    enhancedProb += 0.10; 
  }
...
function App() {
  ...
  // ★ ターンが変わった時に持ち時間（30秒）をリセット
  useEffect(() => {
    setTurnTimeLeft(30);
  }, [turn, exchangePhase, matchCount]);

  // ★ 1秒ごとのカウントダウン（ランク戦のみ）
  useEffect(() => {
    if (currentScreen !== 'game' || matchResultModal || gameResultModal || exchangePhase || !isRankMatch) return;
    ...
    return () => clearInterval(timer);
  }, [turn, currentScreen, matchResultModal, gameResultModal, exchangePhase, isRankMatch]);

  // ★ 持ち時間が0になった場合の自動パス処理
  useEffect(() => {
    if (turnTimeLeft === 0 && isRankMatch && currentScreen === 'game' && !exchangePhase && !matchResultModal && !gameResultModal) {
      if (turn === mySlot) {
        // 自分のターンが時間切れになった場合、自動的にパス
        handleUserPass();
      } else if (isHost && isOnlineMode) {
        // ホストの場合：相手（ゲスト）が時間切れで応答しない場合を考慮し、2秒猶予を与えて強制パスを発動する
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [turnTimeLeft, turn, mySlot, isRankMatch, currentScreen, exchangePhase, matchResultModal, gameResultModal, isHost, isOnlineMode]);
  ...
  const playCards = (playerIdx, cards) => {
    ...
    const isSp3Return = field && field.cards.length === 1 && field.cards[0].num === 0 && cards.length === 1 && cards[0].suit === '♠' && cards[0].num === 3;
    ...
    if (cards.some(c => c.num === 8)) {
      if (isFinished) {
        ...
        advanceTurn(playerIdx, nextHands, newRankings, newFouled, null);
        return;
      } else {
        setMessage(logs.join(' / ') + ' / ✂️ 8切り！ ターン継続');
        clearField(playerIdx, nextHands, newRankings, newFouled); 
        return;
      }
    }
    ...
    if (isSp3Return) {
      if (isFinished) {
        ...
        setMessage(logs.join(' / ') + ' / ♠️ スペ3返し！ ターン継続');
        clearField(playerIdx, nextHands, newRankings, newFouled); 
        return;
      }
    }
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      const isUltraAI = playerRatings[turn] >= 2000;
      const thinkTime = isUltraAI ? Math.floor(Math.random() * 8000) + 7000 : 1000;
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false, allHands = [], currentTurn = 0) => {
    ...
    const smartProb = Math.min(1, cpuRating / 1500); 
    const verySmartProb = Math.min(1, Math.pow(cpuRating / 1800, 2));

    const isSmart = isUltraAI || isEnhancedAI || (Math.random() < smartProb);
    const isVerySmart = isUltraAI || isEnhancedAI || (Math.random() < verySmartProb);
            ...
            <div className="modal-content rules-modal-content">
              ...
              <div className="rules-scroll-area">
                <ul>
                  ...
                  <li><strong>♠️ スペ3返し:</strong> ジョーカー単体出しに対して、♠3単体で勝利でき、そのまま出した人の番になります。</li>
                  ...
                  <li><strong>⏱ 持ち時間:</strong> ランク戦では1手30秒の持ち時間があります。</li>
                </ul>
              </div>
          ...
          <div className="game-board">
            <div className="game-header">
              ...
              {isRankMatch && !exchangePhase && (
                <span style={{ color: turnTimeLeft <= 5 ? '#ff4757' : 'white', fontWeight: 'bold' }}>
                  ⏳ {turnTimeLeft}秒
                </span>
              )}
