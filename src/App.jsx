...
const UPDATE_HISTORY = [
  {
    version: '2.3.1',
    features: [
      '📊 ランキングの順位変動を累計数値表示（↑1, ↓2など）に変更し、12時間ごとにリセット・更新されるように変更しました！'
    ]
  },
  {
    version: '2.3',
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      list.forEach((user, idx) => {
        ...
        let baseRank = user.rankBase12h;
        let baseTime = user.rankBaseUpdatedAt || 0;
        let needsUpdate = false;

        // 基準順位がない、または基準時間から12時間以上経過した場合は最新順位に更新
        if (!baseRank || (now - baseTime > TWELVE_HOURS)) {
          baseRank = currentRank;
          baseTime = now;
          needsUpdate = true;
        }

        // 順位変動（数字）の計算: baseRank - currentRank (例: 5位→3位なら +2 で UP 2)
        const rankDiff = baseRank - currentRank;
        user.rankDiff = rankDiff;
        ...
        if (needsUpdate && user.userId) {
          try {
            updateDoc(doc(db, 'users', user.userId), {
              rankBase12h: baseRank,
              rankBaseUpdatedAt: baseTime
            }).catch(()=>{});
          } catch(e) {}
        }
      });
                  ...
                  return (
                    <div key={idx} className={`ranking-item ${user.userId === userData.userId ? 'highlight-me' : ''}`}>
                      <span style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {user.rankDiff > 0 && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold' }}>↑{user.rankDiff}</span>}
                        {user.rankDiff < 0 && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold' }}>↓{Math.abs(user.rankDiff)}</span>}
                      </span>
              ...
              <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '2px dashed #bdc3c7' }}>
                ...
                <div className="ranking-item highlight-me" style={{ margin: '0', borderRadius: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                    ...
                    {myRankingInfo.user.rankDiff > 0 && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold' }}>↑{myRankingInfo.user.rankDiff}</span>}
                    {myRankingInfo.user.rankDiff < 0 && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold' }}>↓{Math.abs(myRankingInfo.user.rankDiff)}</span>}
                  </span>
