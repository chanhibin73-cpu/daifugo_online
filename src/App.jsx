...
function App() {
  ...
  useEffect(() => {
    const runAISimulation = async () => {
      const aiIds = Array.from({ length: 44 }, (_, i) => `ai_player_${i + 1}`);
      const aiData = [];
      ...
      if (aiData.length === 44) {
        ...
        if (matchCount > 0) {
           ...
           for (let m = 0; m < matchCount; m++) {
              ...
              // シミュレーション時は、isSuperAIの場合レーティング2000相当としてソートの重み付けを行う
              playersForMatch.sort((a, b) => {
                const effA = a.isSuperAI ? 2000 : (a.rating || 500);
                const effB = b.isSuperAI ? 2000 : (b.rating || 500);
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      list.forEach((user, idx) => {
        ...
        // 2日経過しているか、基準順位が未保存なら現在の順位でリセット更新
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    while (currentPlayers.length < 4) {
      if (roomData.type === 'rank' && availableAIList.length > 0) {
        ...
        currentPlayers.push({ userId: ai.userId, name: ai.username, rating: ai.rating, isCpu: true, isAIPlayer: true, isSuperAI: ai.isSuperAI || false });
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      const move = findBestCpuMove(cpuHand, field, isReversedEffective, cpuRating, hands, turn, isAIPlayer, isSuperAI);
      
      const effectiveRating = isSuperAI ? 2000 : (isAIPlayer ? cpuRating * 2 : cpuRating);
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, allHands = [], currentTurn = 0, isAIPlayer = false, isSuperAI = false) => {
