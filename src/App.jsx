...
const UPDATE_HISTORY = [
  {
    version: '2.0.1',
    features: [
      '🎨 UI/UXの切り替え機能を追加しました！（設定画面からテーマをモダン/クラシックに変更可能）',
      '📊 ランキング画面の下部空白エリアに現在の自分の順位・レーティングを表示するようにしました！',
      '🎨 ボタンのカラー変更機能を設定に追加しました！'
    ]
  },
  {
    version: '1.9.5',
...
function App() {
  ...
  const [userData, setUserData] = useState(() => {
    ...
    if (saved) {
      try {
        ...
        if (!parsed.btnColor) parsed.btnColor = '#3498db';
        if (!parsed.uiMode) parsed.uiMode = 'modern';
        return parsed;
      } catch (e) {}
    }
    return {
  ...
  useEffect(() => {
    const runAISimulation = async () => {
      const aiIds = [
        'ai_player_1', 'ai_player_2', 'ai_player_3',
        'ai_player_4', 'ai_player_5', 'ai_player_6',
        'ai_player_7', 'ai_player_8', 'ai_player_9',
        'ai_player_10', 'ai_player_11', 'ai_player_12'
      ];
      const aiData = [];
  ...
  // ボタン用のカスタムスタイル設定
  const btnStyle = { backgroundColor: userData.btnColor || '#3498db', color: '#fff' };

  // 自分のランキング順位（空白部分用）
  const myRankIndex = globalRankings.findIndex(u => u.userId === userData.userId);

  return (
    <div className={`app-outer ${userData.uiMode || 'modern'}`} style={containerStyle}>
      <style>{`
        .card-item.x-small {
          width: 35px;
          height: 50px;
        ...
        .app-outer.classic .menu-btn, .app-outer.classic .action-btn {
      ...
      <div className="app-container">
        ...
        {showInitialSettingsModal && (
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <button className="action-btn" style={btnStyle} onClick={() => {
        ...
        {showUpdateModal && (
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <button className="action-btn" style={btnStyle} onClick={handleCloseUpdate}>確認しました</button>
            </div>
          </div>
        )}
          ...
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <form onSubmit={handleRegister}>
                ...
                <button type="submit" className="action-btn" style={btnStyle}>登録する</button>
              </form>
            </div>
          </div>
        )}
        ...
        {showRecommendModal && (
          <div className="modal-overlay">
            <div className="modal-content recommend-modal">
              ...
              <a href="https://hibimaruwebappscollection.vercel.app/index.html" target="_blank" rel="noopener noreferrer" className="recommend-link-btn" style={btnStyle}>サイトを見る</a>
              <div><button className="action-btn next-modal-btn" style={btnStyle} onClick={handleCloseRecommend}>次へ（ルールを確認）</button></div>
            </div>
          </div>
        )}
        ...
        {showRulesModal && (
          <div className="modal-overlay">
            <div className="modal-content rules-modal-content">
              ...
              <button className="action-btn" style={btnStyle} onClick={() => setShowRulesModal(false)}>ゲームを始める</button>
            </div>
          </div>
        )}
        ...
        {currentScreen === 'menu' && (
          <div className="menu-container">
            ...
            <button className="menu-btn" style={btnStyle} onClick={() => setCurrentScreen('online_select')}>オンライン対戦</button>
            <button className="menu-btn" style={btnStyle} onClick={() => { setMySlot(0); startNewGameSet(['あなた', 'CPU 1', 'CPU 2', 'CPU 3'], false, false); }}>コンピュータ対戦</button>
            <button className="menu-btn" style={btnStyle} onClick={() => setCurrentScreen('ranking')}>全プレイヤーレーティングランキング</button>
            <button className="menu-btn" style={btnStyle} onClick={() => setCurrentScreen('profile')}>プロフィール</button>
            <button className="menu-btn" style={btnStyle} onClick={() => setCurrentScreen('settings')}>設定</button>
          </div>
        )}
        ...
        {currentScreen === 'online_select' && (
          <div className="sub-screen">
            <h2>オンライン対戦</h2>
            <div className="menu-container">
              <button className="menu-btn mode-btn" style={btnStyle} onClick={joinRankMatch}>
                ランク戦
                <span className="btn-subtext">レーティング競い合い / 全国ランキング</span>
              </button>
              <button className="menu-btn mode-btn" style={btnStyle} onClick={() => setCurrentScreen('room_select')}>
                ルーム戦
        ...
        {currentScreen === 'room_select' && (
          <div className="sub-screen">
            <h2>ルーム戦</h2>
            <button className="menu-btn" style={btnStyle} onClick={createRoom}>部屋を作成する</button>
            <div className="room-input-box">
              ...
              <button className="action-btn" style={btnStyle} onClick={joinRoomByCode}>部屋に入る</button>
            </div>
        ...
        {currentScreen === 'waiting_rank' && (
          <div className="sub-screen">
            ...
            <button className="action-btn" style={btnStyle} onClick={startOnlineGameWithCpu}>今すぐ対戦開始 (CPU補充)</button>
        ...
        {currentScreen === 'waiting_room' && (
          <div className="sub-screen">
            ...
            {isHost ? (
              <button className="action-btn" style={btnStyle} onClick={startOnlineGameWithCpu}>試合スタート</button>
            ) : (
                      ...
                      {prevRanks.indexOf(mySlot) === 0 || prevRanks.indexOf(mySlot) === 1 ? (
                        <>
                          ...
                          <button 
                            ...
                            style={{ padding: '6px 12px', fontSize: '12px', ...btnStyle }}
                        ...
                        <>
                          ...
                          <button className="action-btn" style={{ padding: '6px 12px', fontSize: '12px', ...btnStyle }} onClick={handleExchangeSubmit}>確認して渡す</button>
                        </>
                      )}
                    </div>
                  )}
            ...
            {gameResultModal && (
              <div className="modal-overlay">
                <div className="modal-content">
                  ...
                  <button className="action-btn" style={btnStyle} onClick={handleFinishGameAndLeave}>
                    メニューへ戻る
                  </button>
                </div>
              </div>
        ...
        {currentScreen === 'ranking' && (
          <div className="sub-screen">
            ...
            <button className="action-btn" style={{ marginBottom: '15px', padding: '8px 16px', fontSize: '0.95rem', ...btnStyle }} onClick={fetchRankings} disabled={rankingLoading}>
            ...
            <div className="ranking-list" style={{ minHeight: '200px' }}>
              {rankingLoading ? (
            ...
            {/* ★ Ver.2.0.1 追加: ランキング下の空白部分に現在の自分を表示 */}
            <div style={{
              marginTop: '15px',
              padding: '12px 16px',
              ...
              <span>👤 あなたの順位: {myRankIndex !== -1 ? `${myRankIndex + 1}位 (${userData.username})` : '集計中'}</span>
              <span style={{ color: '#27ae60' }}>{userData.rating} Pt</span>
            </div>

            <button className="back-btn" style={{ marginTop: '15px' }} onClick={() => setCurrentScreen('menu')}>戻る</button>
          </div>
        )}
        ...
        {currentScreen === 'settings' && (
          <div className="sub-screen settings-screen">
            ...
            {/* ★ Ver.2.0.1 追加: ボタンの色変更機能 */}
            <div className="setting-item">
              <label>ボタンの色変更</label>
              <input type="color" value={userData.btnColor || '#3498db'} onChange={(e) => saveUserData({ ...userData, btnColor: e.target.value })} />
            </div>

            {/* ★ Ver.2.0.1 追加: UI/UXモード切り替え */}
            <div className="setting-item">
              <label>UI/UXモード（テーマ切替）</label>
              <select value={userData.uiMode || 'modern'} onChange={(e) => saveUserData({ ...userData, uiMode: e.target.value })}>
                <option value="modern">モダン（標準）</option>
                <option value="classic">クラシック（レトロ調）</option>
              </select>
            </div>
            ...
            <div style={{ marginTop: '15px', textAlign: 'center' }}>
              <button className="action-btn" style={{ ...btnStyle, width: '100%' }} onClick={() => setShowUpdateModal(true)}>
                📢 更新情報 (リリースノート) を確認
              </button>
            </div>
