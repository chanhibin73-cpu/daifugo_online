...
const UPDATE_HISTORY = [
  {
    version: '1.4.1',
    features: [
      '🤖 コンピュータの思考プログラム調整: レーティング1000以上のコンピュータの思考回路をより深く改修し、手札温存や他プレイヤーへの妨害判断の精度を向上させました。'
    ]
  },
  {
    version: '1.4',
...
function App() {
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      // Ver.1.4: 超強化AIは最大15秒(7000~15000ms)熟考する
  ...
  // ★ Ver.1.4.1: 1000以上のCPU思考をより深く最適化・判定強化
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false, allHands = [], currentTurn = 0) => {
    ...
    const isHighRatingCPU = cpuRating >= 1000; // Ver.1.4.1: レート1000以上の思考判定
    const isSmart = isUltraAI || isHighRatingCPU || ((Math.random() * 1000) < cpuRating);
    const isVerySmart = isUltraAI || (isHighRatingCPU && Math.random() < 0.85) || (isSmart && ((Math.random() * 1000) < cpuRating));
    ...
    // 高レートCPU (1000以上) および 超強化AI (2000以上) の思考判定
    if (isUltraAI || (isHighRatingCPU && someoneIsFinishing)) {
