...
const UPDATE_HISTORY = [
  {
    version: '1.4.2',
    features: [
      '🐛 バグ修正: スペードの3を出してジョーカーに勝った（スペ3返し）時、その場が流れて出した人の番になるように修正しました。',
      '🤖 コンピュータ調整: 各強化コンピュータの出現確率を上昇（プレイヤーレート1000以上でさらに上昇）させ、思考力がレーティングによってより大きく変化するように調整しました。'
    ]
  },
  {
    version: '1.4.1',
...
// プレイヤーのレーティングに比例したCPUのレーティング生成 (Ver.1.4.2 確率調整)
const generateCpuRating = (playerRating) => {
  ...
  if (pRating >= 1000) {
    enhancedProb += 0.10; // レーティング1000超えでさらに確率上昇ボーナス (+10%)
  }
...
function App() {
  ...
  const playCards = (playerIdx, cards) => {
    ...
    // ★ Ver.1.4.2: 出したカードが「スペ3返し」かどうかの判定
    ...
    // ★ Ver.1.4.2: スペ3返しの判定と処理（8切りと同様に場が流れる）
    if (isSp3Return) {
      if (isFinished) {
        logs.push('♠️ スペ3返し！');
        setMessage(logs.join(' / '));
  ...
  // ★ Ver.1.4.2: CPUの思考力がレーティングによって大きくスケーリングするように変更
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false, allHands = [], currentTurn = 0) => {
    ...
    // レーティングに応じて 0〜1.0 の間で賢くなる確率を厳密に計算
    const smartProb = Math.min(1, cpuRating / 1500); 
    const verySmartProb = Math.min(1, Math.pow(cpuRating / 1800, 2));

    const isSmart = isUltraAI || (Math.random() < smartProb);
    const isVerySmart = isUltraAI || (Math.random() < verySmartProb);
