...
const UPDATE_HISTORY = [
  {
    ...
    features: [
      '🐛 バグ修正を行いました。',
      '📈 階段ルールの追加: マークが同じで数字が連続している3枚以上のカード（例: ♠4,♠5,♠6）を階段として出せるようになりました。'
    ]
  },
  {
    version: '1.6.1',
  ...
  },
  {
    version: '1.6',
    features: [
      '📊 レーティング計算の調整: 低レーティング（800以下）の上昇率を増加し、レーティング高低に応じた増減補正を導入しました。'
    ]
  },
  {
    version: '1.5.1',
...
// 階段判定ヘルパー
const getSequenceInfo = (cards, isRev) => {
  if (cards.length < 3) return { isSeq: false };
  const nonJokers = cards.filter(c => c.num !== 0);
  const jokersCount = cards.length - nonJokers.length;

  if (nonJokers.length > 0) {
    const suit = nonJokers[0].suit;
    if (!nonJokers.every(c => c.suit === suit)) return { isSeq: false };
  }

  if (nonJokers.length === 0) return { isSeq: false }; 

  const strengths = nonJokers.map(c => getCardStrength(c.num, false)).sort((a, b) => a - b);
  const uniqueStrengths = new Set(strengths);
  if (uniqueStrengths.size !== strengths.length) return { isSeq: false };

  const minStr = strengths[0];
  const maxStr = strengths[strengths.length - 1];

  const internalJokersNeeded = (maxStr - minStr + 1) - strengths.length;
  if (internalJokersNeeded > jokersCount) return { isSeq: false };

  let seqStart = minStr;
  let seqEnd = maxStr;
  let remainingJokers = jokersCount - internalJokersNeeded;
  
  while (remainingJokers > 0 && seqEnd < 13) {
    seqEnd++;
    remainingJokers--;
  }
  while (remainingJokers > 0 && seqStart > 1) {
    seqStart--;
    remainingJokers--;
  }
  
  if (remainingJokers > 0) return { isSeq: false };

  let baseStrengths = [];
  for (let s = seqStart; s <= seqEnd; s++) {
    let revStr = isRev ? (14 - s) : s;
    baseStrengths.push(revStr);
  }
  
  const effectiveStrength = Math.min(...baseStrengths);
  return { isSeq: true, strength: effectiveStrength };
};
...
function App() {
  ...
  const isValidPlay = (cards, currentField, isRev) => {
    ...
    if (nonJokers.length > 0) {
      ...
      if (nonJokers.every(c => c.num === targetNum)) isPair = true;
    } else if (jokersCount > 0) {
      isPair = true;
    }

    const seqInfo = getSequenceInfo(cards, isRev);

    let playType = '';
    let playedStrength = 0;

    if (isPair) {
      playType = 'pair';
      const playedNum = nonJokers.length > 0 ? nonJokers[0].num : 0;
      playedStrength = getCardStrength(playedNum, isRev);
    } else if (seqInfo.isSeq) {
      playType = 'sequence';
      playedStrength = seqInfo.strength;
    } else {
      return false;
    }
    ...
    if ((currentField.type === 'pair' || !currentField.type) && currentField.cards.length === 1 && currentField.cards[0].num === 0) {
      if (cards.length === 1 && cards[0].suit === '♠' && cards[0].num === 3) return true;
    }

    if (cards.length !== currentField.cards.length) return false;
    const currentType = currentField.type || 'pair';
    if (playType !== currentType) return false;
  ...
  const playCards = (playerIdx, cards) => {
    ...
    let playType = isPair ? 'pair' : 'sequence';
    let strength = 0;
    ...
    if (isPair) {
      ...
      const seqInfo = getSequenceInfo(cards, effectiveRev);
      strength = seqInfo.strength;
    }
    ...
    const newField = { cards, strength, count: cards.length, playedBy: playerIdx, type: playType };
    setField(newField);
    ...
    const isSpade3Return = field && (field.type === 'pair' || !field.type) && field.cards.length === 1 && field.cards[0].num === 0 && cards.length === 1 && cards[0].suit === '♠' && cards[0].num === 3;

    if (cards.some(c => c.num === 8) || isSpade3Return) {
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false, allHands = [], currentTurn = 0) => {
    ...
    if (!currentField) {
      ...
      SUITS.forEach(suit => {
        const suitCards = nonJokers.filter(c => c.suit === suit).sort((a,b) => getCardStrength(a.num, false) - getCardStrength(b.num, false));
        for (let i = 0; i < suitCards.length; i++) {
          for (let j = i + 2; j < suitCards.length; j++) {
            const seqCandidate = suitCards.slice(i, j + 1);
            if (getSequenceInfo(seqCandidate, isRev).isSeq) {
               options.push(seqCandidate);
               if (jokerCard) options.push([...seqCandidate, jokerCard]);
            }
          }
        }
        if (jokerCard) {
           for (let i = 0; i < suitCards.length; i++) {
             for (let j = i + 1; j < suitCards.length; j++) {
               const seqCandidate = [...suitCards.slice(i, j + 1), jokerCard];
               if (getSequenceInfo(seqCandidate, isRev).isSeq) {
                 options.push(seqCandidate);
               }
             }
      ...
      if (currentField.type === 'sequence' || !currentField.type) {
         SUITS.forEach(suit => {
            const suitCards = nonJokers.filter(c => c.suit === suit).sort((a,b) => getCardStrength(a.num, false) - getCardStrength(b.num, false));
            for (let i = 0; i < suitCards.length; i++) {
              for (let j = i + 1; j < suitCards.length; j++) {
                const seqCandidate = suitCards.slice(i, j + 1);
                if (seqCandidate.length === reqCount && isValidPlay(seqCandidate, currentField, isRev)) options.push(seqCandidate);
                if (jokerCard && seqCandidate.length === reqCount - 1) {
                   const combo = [...seqCandidate, jokerCard];
      ...
      if (currentField.type === 'pair' || !currentField.type) {
          if (reqCount === 1) {
            sorted.forEach(c => {
    ...
    if (isSmart && availableOptions.length > 0) {
      for (let move of availableOptions) {
        ...
        if (remHand.length === 0) {
          ...
          const hasRemJoker = remHand.some(c => c.num === 0);
    ...
    if (isUltraAI || isEnhancedAI) {
      if (someoneIsFinishing) {
        ...
        const noSpecials = availableOptions.filter(combo => !combo.some(c => c.num === 8 || c.num === 11 || c.num === 0 || (isRev ? c.num===3 : c.num===2) || (isUltraAI && combo.length >= 4)));
            ...
            <div className="modal-content rules-modal-content">
              ...
              <div className="rules-scroll-area">
                <ul>
                  ...
                  <li><strong>📈 階段:</strong> マークが同じで数字が連続する3枚以上のカードをまとめて出せます。</li>
          ...
          <div className="sub-screen settings-screen">
            ...
            <div className="setting-rules-box">
              ...
              <ul className="rules-mini-list">
                ...
                <li><strong>📈 階段:</strong> マークが同じで数字が連続する3枚以上のカードをまとめて出せます</li>
                <li><strong>⚠️ 都落ち:</strong> 前回大富豪未達成で最下位</li>
