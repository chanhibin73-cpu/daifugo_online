...
function App() {
  ...
  const [syncedGameOver, setSyncedGameOver] = useState(false); // ゲストの二重処理防止フラグ
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      if (data) {
        ...
        if (data.status === 'playing' && currentScreen.startsWith('waiting')) {
          ...
          // ゲストは画面遷移と情報セットのみ行い、手札生成(startNewGameSet)は行わない！
          if (data.hostId !== userData.userId) {
            setPlayers(data.players.map(p => p.name));
            setPlayerRatings(data.players.map(p => p.rating));
            setIsOnlineMode(true);
            setIsRankMatch(data.type === 'rank');
          }
        }
      } else {
        // ホスト退出等で部屋が消滅した場合、自動退出
  ...
  const joinRankMatch = async () => {
    ...
    if (targetRoomId) {
      setMySlot(targetRoom.players.length);
  ...
  const executeStartOnlineMatch = async (rId, playerList) => {
    ...
    setCpuLevels(playerList.map(p => p.isCpu ? Math.random() < 0.2 : false));
    setPlayers(playerList.map(p => p.name));
    setPlayerRatings(playerList.map(p => p.rating));
    // ホストのみ新規ゲームセットを初期化実行する
    startNewGameSet(playerList.map(p => p.name), isRankMatch, true);
  };
  ...
  const startNewGameSet = (playerList = ['あなた', 'CPU 1', 'CPU 2', 'CPU 3'], isRank = false, isOnline = false) => {
    if (!isOnline) setCpuLevels([false, Math.random() < 0.2, Math.random() < 0.2, Math.random() < 0.2]);
  ...
  const findBestCpuMove = (hand, currentField, isRev, isEnhanced) => {
    ...
    if (!currentField) {
      if (isEnhanced) {
        for (let num in groups) { if (num != 8 && groups[num].length >= 2 && !isForbiddenFinish(groups[num])) return groups[num]; }
  ...
  const handleConfirmReturnHome = async () => {
    ...
    if (isOnlineMode && roomId && roomData) {
      ...
      if (updatedPlayers[mySlot]) updatedPlayers[mySlot] = { userId: `cpu_${mySlot}`, name: `CPU (代行)`, rating: 500, isCpu: true };
      await update(ref(rtdb, `rooms/${roomId}`), { players: updatedPlayers });
