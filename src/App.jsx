...
function App() {
  ...
  // 強化コンピュータ判定（20%の確率で賢いCPUになる。true=強化、false=通常）
  ...
  const [fouledPlayers, setFouledPlayers] = useState([]); // 反則上がり（最下位確定）プレイヤー
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      let errMsg = 'ランキングデータの取得に失敗しました。';
      if (err.code === 'permission-denied') {
        errMsg = 'アクセス権限エラー: Firebaseのセキュリティルールを確認してください。';
      } else if (err.message) {
        errMsg += ` (${err.message})`;
      }
  ...
  const advanceTurn = (
    ...
    // 次のプレイヤー探索（上がり済みの人のみスキップ、パスした人は手番が回るように変更）
    let next = (currentIdx + 1) % 4;
  ...
  }, [turn, field, hands, currentScreen, isReversedEffective, matchResultModal, roomData, isOnlineMode, cpuLevels]);

  // CPU思考：強化CPUはペアを崩さず、初手で安易に8切りを消費しない深い思考をする
  ...
  const endMatch = (finalRankings, finalHands, currentFouled = fouledPlayers) => {
    ...
    // 通常上がり順 -> 残りプレイヤー -> 反則者（反則発生順の逆順で最下位）
      ...
      <div className="app-container">
          ...
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <form onSubmit={handleRegister}>
                <input
                  type="text"
                  className="large-username-input"
                  style={{
                    fontSize: '1.2rem',
                    padding: '12px 16px',
                    width: '90%',
        ...
        {currentScreen === 'ranking' && (
          <div className="sub-screen">
            ...
            <button
              className="action-btn"
              style={{ marginBottom: '15px', padding: '8px 16px', fontSize: '0.95rem' }}
            ...
            {rankingError && (
              <p className="error-text" style={{ color: '#e74c3c', padding: '10px', background: 'rgba(231,76,60,0.1)', borderRadius: '6px' }}>
                {rankingError}
              </p>
            )}
        ...
        {currentScreen === 'settings' && (
          <div className="sub-screen settings-screen">
            ...
            <div className="setting-item">
              ...
              <input
                type="text"
                style={{ fontSize: '1.1rem', padding: '8px 12px', width: '100%', boxSizing: 'border-box' }}
