...
function App() {
  ...
  // ★ Ver3.4 ホスト側のキュー処理用ロック
  const [isProcessingHostReq, setIsProcessingHostReq] = useState(false);
  ...
  useEffect(() => {
    if (turn === mySlot || exchangePhase || field === null) {
      setIsProcessing(false);
    }
  ...
  useEffect(() => {
    if (isOnlineMode && isHost && requestsQueue.length > 0 && !isProcessingHostReq) {
      setIsProcessingHostReq(true);
      ...
      remove(ref(rtdb, `rooms/${roomId}/requests/${req.id}`)).then(() => {
        setTimeout(() => setIsProcessingHostReq(false), 200);
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [requestsQueue, isProcessingHostReq, isOnlineMode, isHost, roomId]);
  ...
  const isValidPlay = (cards, currentField, isRev) => {
    ...
    if (currentField.isBound) {
      ...
      if (matchCount + jokerCount < prevSuits.length) {
         return false;
      }
    }
  ...
  const playCards = (playerIdx, cards) => {
    ...
    if (field) {
      ...
      if (isSameSuit && matchCount + jokerCount >= prevSuits.length) {
         isSameSuit = true;
      } else {
         isSameSuit = false;
      }
      ...
      if (newSuits.length === 0 && jokerCount > 0) {
         newSuits = ['🃏']; 
      }
    }
    ...
    if ((!isSeq && cards.some(c => c.num === 8)) || isSpade3Return) {
      ...
      if (isFinished) {
        ...
        clearField(playerIdx, nextHands, newRankings, newFouled); 
        setField(null); // 明示的にクリア
        setPassed([...nextPassed].map(() => false));
        return;
      }
    }
  ...
  const clearField = (nextTurnPlayer, currentHands, currentRankings, currentFouled = fouledPlayers) => {
    ...
    let loop = 0;
    while (currentHands[next].length === 0 && loop < 4) {
      next = (next + 1) % 4;
      loop++;
    }
    setTurn(next);
  ...
  const advanceTurn = (currentIdx, currentHands, currentRankings, currentFouled = fouledPlayers, currentField, currentPassed = passed) => {
    ...
    let nextTurn = (currentIdx + 1) % 4;

    if (currentField) {
      if (activePlayers.includes(currentField.playedBy)) {
        ...
        if (othersPassedCount >= othersCount) {
          shouldClear = true;
          nextTurn = currentField.playedBy;
        }
      } else {
        if (activePassed.length >= activePlayers.length) {
          shouldClear = true;
          let tempNext = (currentField.playedBy + 1) % 4;
          let loopCount = 0;
          while (!activePlayers.includes(tempNext) && loopCount < 4) {
             tempNext = (tempNext + 1) % 4;
             loopCount++;
          }
          nextTurn = tempNext;
        }
      ...
      if (activePassed.length >= activePlayers.length) {
        shouldClear = true;
        nextTurn = (currentIdx + 1) % 4;
      }
    }

    if (shouldClear) {
      clearField(nextTurn, currentHands, currentRankings, currentFouled);
      return;
    }
