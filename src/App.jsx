...
const UPDATE_HISTORY = [
  {
    version: '1.6',
    features: [
      '🪜 新ルール「階段」追加: 同じマークで数字が連続したカードを3枚以上同時に出せるようになりました。',
      '🔥 階段革命: 階段を4枚以上同時に出すと「階段革命」が発生し、強さが逆転します。',
      '⚠️ 階段の特殊ルール: 階段の中に8、11、♠3を含んでいても、8切り・11バック・スペ3返しの効果は発動しません。'
    ]
  },
  {
    version: '1.5.2',
    features: [
      '🤖 コンピュータ思考時間の修正: 超強化コンピュータの思考時間を状況に応じて柔軟に変化させ、出せるカードがない場合はすぐにパスするようにテンポを改善しました。'
    ]
  },
  {
    version: '1.5.1',
  ...
  },
  {
    version: '1.4.1',
    features: [
      '🤖 コンピュータ思考のアップデート: レーティング1000以上のコンピュータの思考をより深くし、他プレイヤーの妨害やカード温存を的確に行うように修正しました。'
    ]
  },
  {
    version: '1.4',
    features: [
      '🤖 コンピュータ思考ルーチンの大改革: すべてのコンピュータが「上がり切るまでの最短手順」を考慮して手を選択するようになりました（精度はレーティングに比例）。',
      '🃏 JOKERワイルドカード機能の強化: コンピュータがJOKERをペアやセットの補填（併用）として活用できるようになりました。',
      '🔥 超強化コンピュータの追加: プレイヤーレート1200以上で5%の確率で出現！最大15秒間の思考で手札状況・他プレイヤーの妨害を判断します。'
    ]
  }
];
...
// 大富豪としての絶対的な「ベース強さ」(3=1, 4=2 ... 2=13)
const getBaseStrength = (num) => (num >= 3) ? num - 2 : num + 11;

const getCardStrength = (num, isReversed) => {
  if (num === 0) return 14;
  let baseStrength = getBaseStrength(num);
  return isReversed ? 14 - baseStrength : baseStrength;
};

// 階段判定ロジック
const parseSequence = (cards) => {
  if (cards.length < 3) return null;
  const jokers = cards.filter(c => c.num === 0);
  ...
  const bases = nonJokers.map(c => getBaseStrength(c.num));
  const uniqueBases = new Set(bases);
  if (uniqueBases.size !== bases.length) return null; // 重複がある場合は階段にならない

  let bestStart = -1;
  // 階段の起点を1〜13の中で探索し、出されたカードを含有できるかチェック
  for (let start = 1; start <= 13 - cards.length + 1; start++) {
     let valid = true;
     for (let b of bases) {
        if (b < start || b >= start + cards.length) {
           valid = false;
           break;
        }
     }
     if (valid) {
        bestStart = start; // 可能な最大の開始位置（強さ）
     }
  }
...
function App() {
  ...
  // ★ Ver.1.6 階段判定＆バリデーションの統合
  const isValidPlay = (cards, currentField, isRev) => {
    if (cards.length === 0) return false;
    
    const seqData = parseSequence(cards);
    const nonJokers = cards.filter(c => c.num !== 0);
    const isNormalMulti = nonJokers.length > 0 && nonJokers.every(c => c.num === nonJokers[0].num);

    if (!seqData && !isNormalMulti) return false;

    if (!currentField) return true;

    if (seqData) {
      // 出すカードが「階段」の場合
      if (!currentField.isSequence) return false; // 場が通常出しなら不可
      if (currentField.count !== cards.length) return false; // 枚数が違うなら不可
      if (isRev) {
        return seqData.baseStart < currentField.baseStart;
      } else {
        return seqData.baseStart > currentField.baseStart;
      }
    } else {
      // 出すカードが「通常出し」の場合
      if (currentField.isSequence) return false; // 場が階段なら不可
      if (currentField.count !== cards.length) return false;

      const playedNum = nonJokers[0].num;
      const playedStrength = getCardStrength(playedNum, isRev);

      if (currentField.count === 1 && currentField.cards[0].num === 0) {
  ...
  const playCards = (playerIdx, cards) => {
    ...
    if (cards.length >= 4) { 
      ...
      logs.push(isSequence ? '🔥 階段革命発生！' : '🔥 革命発生！'); 
    }
    
    // 11バックは階段では発動しない
    let next11Back = is11Back;
    if (!isSequence && cards.some(c => c.num === 11)) { 
      next11Back = true; 
      setIs11Back(true); 
      logs.push('↺ 11バック発生！'); 
    }
    ...
    let baseStart = 0;
    if (isSequence) {
      baseStart = seqData.baseStart;
    } else {
      strength = getCardStrength(playedNum, effectiveRev);
    }
    ...
    if (isFinished) {
      // 階段の時は特殊効果がないため、8上がりなどは禁止にならない（JOKER/最強カードも同様に効果なしとみなすルールに準拠）
      const contains8 = !isSequence && cards.some(c => c.num === 8);
      const containsJoker = !isSequence && cards.some(c => c.num === 0);
      const containsForbiddenStrong = !isSequence && (effectiveRev ? cards.some(c => c.num === 3) : cards.some(c => c.num === 2));
    ...
    // スペ3返し・8切りは階段では発動しない
    const isSpade3Return = !isSequence && field && field.count === 1 && field.cards[0].num === 0 && cards.length === 1 && cards[0].suit === '♠' && cards[0].num === 3;

    if (!isSequence && (cards.some(c => c.num === 8) || isSpade3Return)) {
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      const move = findBestCpuMove(cpuHand, field, isReversedEffective, cpuRating, isSubstitute, hands, turn);
      
      const isUltraAI = cpuRating >= 2000;
      let thinkTime = 1000;
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false, allHands = [], currentTurn = 0) => {
    ...
    const isUltraAI = cpuRating >= 2000;
    const isEnhancedAI = cpuRating >= 1000; 
    const isSmart = isUltraAI || isEnhancedAI || ((Math.random() * 1000) < cpuRating);
    const isVerySmart = isUltraAI || isEnhancedAI || (isSmart && ((Math.random() * 1000) < cpuRating));

    const isForbiddenFinish = (cards) => {
      const isSeq = !!parseSequence(cards);
      if (isSeq) return false; // 階段の時は特殊効果がないので禁止上がりにはならない
    ...
    // 階段の組み合わせを探索するヘルパー
    const findSequences = (handCards) => {
        const seqs = [];
        const jokers = handCards.filter(c => c.num === 0);
        for (let suit of SUITS) {
            const suitCards = handCards.filter(c => c.suit === suit);
            for (let start = 1; start <= 11; start++) {
                for (let len = 3; len <= 13 - start + 1; len++) {
                    let combo = [];
                    let availableJokers = [...jokers];
                    for (let i = 0; i < len; i++) {
                        const targetBase = start + i;
                        const targetNum = targetBase <= 11 ? targetBase + 2 : targetBase - 11;
                        const card = suitCards.find(c => c.num === targetNum);
                        if (card) {
                            combo.push(card);
                        } else if (availableJokers.length > 0) {
                            combo.push(availableJokers.pop());
                        } else {
                            combo = null;
                            break;
                        }
                    }
                    if (combo) seqs.push(combo);
                }
            }
        }
        return seqs;
    ...
    if (!currentField) {
      ...
      options.push(...findSequences(hand));
    } else {
      const reqCount = currentField.count;

      if (currentField.isSequence) {
        const seqs = findSequences(hand).filter(s => s.length === reqCount);
        seqs.forEach(s => {
          if (isValidPlay(s, currentField, isRev)) options.push(s);
        });
      } else {
    ...
    if (isUltraAI || isEnhancedAI) {
      if (someoneIsFinishing) {
        availableOptions.sort((a, b) => {
          const aS = parseSequence(a) ? parseSequence(a).baseStart : getCardStrength(a[0].num, isRev);
          const bS = parseSequence(b) ? parseSequence(b).baseStart : getCardStrength(b[0].num, isRev);
          return bS - aS;
        });
        const eights = availableOptions.filter(combo => combo.some(c => c.num === 8) && !parseSequence(combo));
        ...
        const noSpecials = availableOptions.filter(combo => {
           const isSeq = !!parseSequence(combo);
           if (isSeq) return combo.length < 4; // 階段の場合は革命(4枚以上)を温存
           return !combo.some(c => c.num === 8 || c.num === 11 || c.num === 0 || (isRev ? c.num===3 : c.num===2) || combo.length >= 4);
        });
        
        if (!currentField) {
           const seqs = noSpecials.filter(combo => !!parseSequence(combo));
           if (seqs.length > 0) return seqs[seqs.length - 1];
           const multiples = noSpecials.filter(combo => combo.length > 1 && !parseSequence(combo));
      ...
      const noSpecials = availableOptions.filter(combo => !combo.some(c => c.num === 8 || c.num === 0) || !!parseSequence(combo));
            ...
            <div className="modal-content rules-modal-content">
              ...
              <div className="rules-scroll-area">
                <ul>
                  ...
                  <li><strong>🔥 革命 / 階段革命:</strong> 4枚以上の同数カード、または4枚以上の階段を出すと即座に強さが逆転します。</li>
                  <li><strong>🪜 階段:</strong> 同じマークで数字が連続したカードを3枚以上同時に出せます。（階段に特殊効果はありません）</li>
          ...
          <div className="sub-screen settings-screen">
            ...
            <div className="setting-rules-box">
              ...
              <ul className="rules-mini-list">
                ...
                <li><strong>🔥 革命 / 階段革命:</strong> 4枚以上の同数/階段出しで即座に強さ反転</li>
                <li><strong>🪜 階段:</strong> 同マークで連続した数字を3枚以上同時出し（特殊効果なし）</li>
                <li><strong>✂️ 8切り:</strong> 8を出して場を流す</li>
                <li><strong>↺ 11バック:</strong> Jを出してターン中強さ反転</li>
                <li><strong>♠️ スペ3返し:</strong> ジョーカー単体に♠3で勝利</li>
