...
// プレイヤーのレーティングに比例したCPUのレーティング生成 (Ver.1.4: レート1200以上で5%の確率で2000以上超強化CPU)
...
function App() {
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      // Ver.1.4: 超強化AIは最大15秒(15000ms)のランダム思考時間で熟考
  ...
  // ★ Ver.1.4: 勝利までの手を思考・JOKER併用思考を組み込んだ強化AIロジック
  const findBestCpuMove = (hand, currentField, isRev, cpuRating, isSubstitute = false, allHands = [], currentTurn = 0) => {
    ...
    const isSmart = isUltraAI || ((Math.random() * 1000) < cpuRating);
    const isVerySmart = isUltraAI || (isSmart && ((Math.random() * 1000) < cpuRating));
    ...
    // --- 勝利までの最短プラン（勝ち手）があるかどうかの思考アルゴリズム ---
    // 手札から特定の1手を打った後、残りの手札が全て一気に上がり切れるセット構成になっているか判定
    if (isSmart && availableOptions.length > 0) {
      for (let move of availableOptions) {
        const remHand = hand.filter(c => !move.some(mc => mc.id === c.id));
        if (remHand.length === 0) {
          if (!isForbiddenFinish(move)) return move; // これで即上がり
        } else {
          // 残り手札が単一グループ（同数カードやJOKER併用）だけで上がり切れるか
    ...
    if (isUltraAI) {
      if (someoneIsFinishing) {
        // 他プレイヤーが上がりそう：全力で阻止（強いカード・8切り・革命を優先）
