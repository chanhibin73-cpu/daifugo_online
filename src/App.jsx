    ...
    const runAISimulation = async () => {
      ...
      if (aiData.length === 71) {
        ...
        for (let ai of simPlayers) {
           ...
           if (isRestTime && ai.status !== 'offline') {
               ai.status = 'offline';
               needsUpdate = true;
           }
        }
              ...
              {rankingLoading ? (
                ...
                globalRankings.map((user, idx) => {
                  const getStatusColor = (u) => {
                    ...
                    if (u.isAIPlayer) {
                      if (isRestTime) return '#e74c3c'; // 休憩中はオフライン
                      if (u.status === 'playing') {
                        // DB上でplayingのまま1時間以上経過していたらオンラインに戻す
                        if (now - (u.updatedAt || 0) > 60 * 60 * 1000) return '#2ecc71';
                        return '#f1c40f';
                      }
                  ...
                  const getStatusText = (u) => {
                    ...
                    if (u.isAIPlayer) {
                      if (isRestTime) return 'オフライン';
                      if (u.status === 'playing') {
                        if (now - (u.updatedAt || 0) > 60 * 60 * 1000) return 'オンライン';
                        return '試合中';
                      }
