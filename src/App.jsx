...
const calcRating = (myRating, opponentRatings, rank) => {
  ...
  let baseChange = 0;
  if (rank === 0) baseChange = 16;
  else if (rank === 1) baseChange = 6;
  else if (rank === 2) baseChange = -6;
  else if (rank === 3) baseChange = -16;
...
function App() {
  ...
  // ホスト判定（オンライン時は部屋作成者がホスト、オフライン時は自分がホスト）
  ...
  const [exchangeCards, setExchangeCards] = useState({}); // { playerIdx: [カード, ...] }
  ...
  const handleDeleteAllData = async () => {
    try {
      if (userData.userId) await deleteDoc(doc(db, 'users', userData.userId));
    } catch (err) {}
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      if (data) {
        ...
        if (data.status === 'playing' && currentScreen.startsWith('waiting')) {
          ...
          // ホスト以外は画面遷移だけ行い、実際のゲーム状態は後述の gameState 同期で降ってくる
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    while (currentPlayers.length < 4) {
      currentPlayers.push({ userId: `cpu_${cpuCount}`, name: `CPU ${cpuCount++}`, rating: 500, isCpu: true });
    }
  ...
  // --- ★ オンライン状態の同期（ホスト・ゲストモデル） ★ ---

  // ① ホスト: ローカルの最新のゲーム状態を Firebase にアップロード
  useEffect(() => {
  ...
  }, [hands, turn, field, passed, rankingsThisMatch, fouledPlayers, isRevolution, is11Back, message, matchCount, scores, prevRanks, exchangePhase, exchangeCards, isOnlineMode, isHost, roomId, currentScreen]);

  // ② ゲスト: Firebase のゲーム状態をローカルにダウンロードして反映
  useEffect(() => {
    if (isOnlineMode && !isHost && roomId && currentScreen === 'game') {
  ...
  }, [isOnlineMode, isHost, roomId, currentScreen]);

  // ③ ホスト: ゲストからのアクションリクエストを受け取るキュー処理
  ...
  const startNewGameSet = (playerList = ['あなた', 'CPU 1', 'CPU 2', 'CPU 3'], isRank = false, isOnline = false) => {
    ...
    setScores([0, 0, 0, 0]); setPrevRanks([0, 1, 2, 3]); setCurrentScreen('game');
  ...
  // ホスト（またはオフライン）がCPUの交換カードを自動選択する
  useEffect(() => {
    if (exchangePhase && (!isOnlineMode || isHost)) {
      ...
      for (let idx = 0; idx < 4; idx++) {
        ...
        if (isCpuSlot && !newEx[idx]) {
          ...
          if (rank === 0 || rank === 1) {
            // CPUの1位, 2位は一番弱いカードを選ぶ
            newEx[idx] = hands[idx].slice(0, reqCount);
          } else {
            // 3位, 4位は一番強いカードを選ぶ
            newEx[idx] = hands[idx].slice(-reqCount);
          }
          changed = true;
        }
      }
  ...
  useEffect(() => {
    if (exchangePhase && Object.keys(exchangeCards).length === 4) {
      if (!isOnlineMode || isHost) {
        ...
        setExchangePhase(false);
        setExchangeCards({});
        setTurn(daihinmin); // 交換完了後は大貧民からスタート
  ...
  const handleExchangeSubmit = () => {
    ...
    if (myRank === 2 || myRank === 3) {
      // 貧民、大貧民は自動的に最強カードを選択
      cardsToSubmit = hands[mySlot].slice(-reqCount);
    }

    if (isOnlineMode && !isHost) {
      push(ref(rtdb, `rooms/${roomId}/requests`), {
        type: 'exchange', playerIdx: mySlot, cards: cardsToSubmit, ts: Date.now()
      });
    } else {
  ...
  const clearField = (nextTurnPlayer, currentHands, currentRankings, currentFouled = fouledPlayers) => {
    ...
    while (
      currentHands[next].length === 0 &&
      (currentRankings.length + currentFouled.length) < 3
    ) {
      next = (next + 1) % 4;
    }
    setTurn(next);
  ...
  useEffect(() => {
    ...
    const isCurrentCpuSlot = isOnlineMode
      ? (isHost && roomData?.players && roomData.players[turn]?.isCpu)
      : (turn !== 0);
  ...
  }, [turn, field, hands, currentScreen, isReversedEffective, matchResultModal, roomData, isOnlineMode, cpuLevels, isHost, exchangePhase]);
  ...
  const findBestCpuMove = (hand, currentField, isRev, isEnhanced) => {
    ...
    const isForbiddenFinish = (cards) => {
      ...
      const contains8 = cards.some(c => c.num === 8);
      const containsJoker = cards.some(c => c.num === 0);
      const containsForbiddenStrong = isRev
        ? cards.some(c => c.num === 3)
        : cards.some(c => c.num === 2);
    ...
    if (!currentField) {
      if (isEnhanced) {
        for (let num in groups) {
          if (num != 8 && groups[num].length >= 2 && !isForbiddenFinish(groups[num])) {
            return groups[num];
          }
        }
        for (let i = 0; i < sorted.length; i++) {
          const card = sorted[i];
          if (card.num !== 0 && card.num !== 8 && groups[card.num].length === 1 && !isForbiddenFinish([card])) {
            return [card];
          }
        }
      }
  ...
  const finish5MatchGameSet = (finalScores) => {
    ...
    // ルーム戦ではレーティング増減は行われない（isRankMatchのみ実行）
    if (isRankMatch) {
  ...
  // アクション送信（ゲスト時はホストへリクエスト、ホスト時は直接実行）
