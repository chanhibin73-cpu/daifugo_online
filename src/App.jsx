...
const UPDATE_HISTORY = [
  {
    version: '3.4',
    features: [
      '📈 ランキングで順位変動がない場合「→（±0）」と表記するようにしました。',
      '⚠️ 放置ペナルティシステムを刷新しました。(2日未ログインから開始し、日数経過で減少量が増加します)'
    ]
  },
  {
    version: '3.3.1',
...
function App() {
  ...
  // ★ Ver3.4 放置ペナルティチェック (2日以上で n=0開始, 減少量 2^n)
  useEffect(() => {
    const now = Date.now();
    ...
    if (daysPassed >= 2 && (userData.rating5 > 0 || userData.rating10 > 0)) {
      let decayAmount = 0;
      for (let i = 2; i <= daysPassed; i++) {
          let n = i - 2;
          if (n > 5) n = 5;
          decayAmount += Math.pow(2, n);
      }
