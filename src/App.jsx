...
import { collection, doc, setDoc, deleteDoc, getDocs, getDoc, updateDoc, arrayUnion, arrayRemove } from 'firebase/firestore';
...
const UPDATE_HISTORY = [
  ...
  },
  {
    version: '2.3',
    features: [
      '📊 ランキングにて、順位変動があったプレイヤーに矢印を表示する機能を追加しました！'
    ]
  },
  {
    version: '2.2.1',
...
// レーティング計算ロジック（NaN汚染を防ぐための防御処理を追加）
const calcRating = (myRating, allRatings, myRank, finalRankings) => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    // 対戦相手のレーティングが万が一不正な値だった場合は500として計算
...
function App() {
  ...
  useEffect(() => {
    ...
    updateMyStatus();
    // 3分に1回ステータスを更新し、ランキング上でオフライン(赤)になるのを防ぐ
  ...
  useEffect(() => {
    const runAISimulation = async () => {
      const aiIds = Array.from({ length: 71 }, (_, i) => `ai_player_${i + 1}`);
      const aiData = [];
      ...
      if (aiData.length === 71) {
        ...
        if (matchCount > 0 && !isRestTime) {
           ...
           for (let mCount = 0; mCount < matchCount; mCount++) {
              // 隠し更新: 強化AI/超強化AIも含めたすべてのAIを対象にシャッフル
              const shuffledAI = [...simPlayers].sort(() => Math.random() - 0.5);
              let playersForMatch = [];
              
              // 隠し更新: AI同士(4人)の対戦を確率で許可する
              if (Math.random() < 0.5) {
              ...
              // シミュレーション時は、isSuperAIやisUltraSuperAIの場合に重み付けを行うことで強さを反映
    ...
    const initAIPlayers = async () => {
      const aiPlayers = [
        ...
        { userId: 'ai_player_43', username: 'AI_Andromeda', rating: 500, maxRating: 500, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' },
        { userId: 'ai_player_44', username: 'AI_Cassiopeia', rating: 500, maxRating: 500, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' },
        { userId: 'ai_player_45', username: 'AI_Centaurus', rating: 450, maxRating: 450, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' },
        { userId: 'ai_player_46', username: 'AI_asn', rating: 1, maxRating: 1, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isUltraSuperAI: true, status: 'online' },
        { userId: 'ai_player_47', username: 'AI_Vulpecula', rating: 500, maxRating: 500, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' },
        { userId: 'ai_player_48', username: 'AI_Monoceros', rating: 600, maxRating: 600, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' },
        { userId: 'ai_player_49', username: 'AI_Camelopardalis', rating: 700, maxRating: 700, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' },
        { userId: 'ai_player_50', username: 'AI_Lynx', rating: 800, maxRating: 800, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' },
        { userId: 'ai_player_51', username: 'AI_Auriga', rating: 900, maxRating: 900, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' },
        { userId: 'ai_player_52', username: 'AI_Bootes', rating: 1000, maxRating: 1000, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' },
        { userId: 'ai_player_53', username: 'AI_Hercules', rating: 1100, maxRating: 1100, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, status: 'online' },
        ...
        { userId: 'ai_player_67', username: 'AI_Vela', rating: 100, maxRating: 100, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' },
        { userId: 'ai_player_68', username: 'AI_Puppis', rating: 200, maxRating: 200, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' },
        { userId: 'ai_player_69', username: 'AI_Carina', rating: 300, maxRating: 300, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' },
        { userId: 'ai_player_70', username: 'AI_Pyxis', rating: 400, maxRating: 400, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' },
        { userId: 'ai_player_71', username: 'AI_Hydra', rating: 500, maxRating: 500, wins: [0,0,0,0], totalGames: 0, currentWinStreak: 0, maxWinStreak: 0, isAIPlayer: true, isSuperAI: true, status: 'online' }
      ];
      let needsInit = false;
      for (const ai of aiPlayers) {
        try {
          ...
          if (!docSnap.exists()) {
            ...
            // ★過去のバグ等によるNaN汚染を検知し、初期値に修復する防御処理
  ...
  useEffect(() => {
    const isAnyModalOpen = showRegisterModal || showRecommendModal || showRulesModal || showUpdateModal || showDisconnectPenaltyModal || showAccountPromptModal || showAccountRegisterModal || showLoginModal;
    if (userData.username && !userData.hasCompleted163Settings && !isAnyModalOpen) {
      setShowInitialSettingsModal(true);
    }
  }, [userData.username, userData.hasCompleted163Settings, showRegisterModal, showRecommendModal, showRulesModal, showUpdateModal, showDisconnectPenaltyModal, showAccountPromptModal, showAccountRegisterModal, showLoginModal]);
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    if (roomData.type === 'rank' && !isRestTime) {
      try {
        const snap = await getDocs(collection(db, 'users'));
        snap.forEach(d => {
          const data = d.data();
    ...
    while (currentPlayers.length < 4) {
      if (roomData.type === 'rank' && availableAIList.length > 0 && !isRestTime) {
  ...
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      ...
      // ★ 意図的な待機時間を削減し、最低限のディレイでターンを進行させる
      const thinkTime = 400; // UI更新や他プレイヤーの視認性確保のための短い固定ディレイ(ms)
  ...
  const sendInvitation = async (friendId, type) => {
    ...
    if (!targetRoomId) {
      const code = Math.floor(1000 + Math.random() * 9000).toString();
      targetRoomId = type === 'rank' ? push(ref(rtdb, 'rooms')).key : `room_${code}`;
      setRoomId(targetRoomId);
      setIsRankMatch(type === 'rank');
      setIsOnlineMode(true);
      setMySlot(0);
      ...
      setCurrentScreen(type === 'rank' ? 'waiting_rank' : 'waiting_room');
    }
  ...
  const acceptInvitation = async (inv) => {
    ...
    setCurrentScreen(inv.type === 'rank' ? 'waiting_rank' : 'waiting_room');
    remove(ref(rtdb, `invitations/${userData.userId}/${inv.id}`));
  };
  ...
  return (
    <div className={`app-outer ${userData.uiMode === 'desktop' ? 'desktop-mode' : ''}`} style={containerStyle}>
      <style>{`
        ...
        .friend-tabs button.active { border-bottom: 3px solid #3498db; color: #3498db; }
        ...
        .btn-chat { background-color: #9b59b6; }
        .btn-invite-rank { background-color: #e67e22; }
        .btn-invite-room { background-color: #3498db; }
        ...
        .chat-message.mine { align-self: flex-end; background-color: #3498db; color: #fff; border-bottom-right-radius: 2px; }
        ...
        <div className="invitation-banner">
          {invitations.map(inv => (
            <div key={inv.id} className="invitation-item">
              <span>📩 <strong>{inv.fromName}</strong> さんから {inv.type === 'rank' ? 'ランク戦' : 'ルーム戦'} の招待が届いています！</span>
                  ...
                  friendsList.map(f => (
                    <div key={f.userId} className="friend-list-item">
                      ...
                      <div className="friend-actions">
                        ...
                        <button className="btn-invite-rank" onClick={() => sendInvitation(f.userId, 'rank')}>ランクへ招待</button>
          ...
          <div className="sub-screen profile-screen">
            ...
            <div className="profile-card">
              ...
              <div className="profile-tabs" style={{ display: 'flex', justifyContent: 'space-around', marginBottom: '15px' }}>
                <button 
                  style={{ flex: 1, padding: '8px', borderBottom: profileTab === 'rank' ? '3px solid #3498db' : 'none', background: 'none', border: 'none', color: profileTab === 'rank' ? '#3498db' : '#7f8c8d', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
                  onClick={() => setProfileTab('rank')}
                >ランク戦</button>
                <button 
                  style={{ flex: 1, padding: '8px', borderBottom: profileTab === 'room' ? '3px solid #3498db' : 'none', background: 'none', border: 'none', color: profileTab === 'room' ? '#3498db' : '#7f8c8d', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
                  onClick={() => setProfileTab('room')}
                >ルーム戦</button>
                <button 
                  style={{ flex: 1, padding: '8px', borderBottom: profileTab === 'cpu' ? '3px solid #3498db' : 'none', background: 'none', border: 'none', color: profileTab === 'cpu' ? '#3498db' : '#7f8c8d', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
