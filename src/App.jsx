...
const UPDATE_HISTORY = [
  {
    version: '2.4',
    features: [
      '📊 ランキングの順位変動を3時間ごとの累計（変動幅の数値付き）に変更しました！'
    ]
  },
  {
    version: '2.3.1',
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      list.forEach((user, idx) => {
        ...
        let baselineTime = user.baselineTime || now;
        let diff = 0;
        let needsUpdate = false;

        if (now - baselineTime >= threeHoursMs) {
          baselineRank = currentRank;
          baselineTime = now;
          needsUpdate = true;
        }

        diff = baselineRank - currentRank; 
                  ...
                  return (
                    <div key={idx} className={`ranking-item ${user.userId === userData.userId ? 'highlight-me' : ''}`}>
                      <span style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {user.rankDiff > 0 && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold', fontSize: '0.85rem' }}>↑{user.rankDiff}</span>}
                        {user.rankDiff < 0 && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold', fontSize: '0.85rem' }}>↓{Math.abs(user.rankDiff)}</span>}
                      </span>
              ...
              <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '2px dashed #bdc3c7' }}>
                ...
                <div className="ranking-item highlight-me" style={{ margin: '0', borderRadius: '8px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                    ...
                    {myRankingInfo.user.rankDiff > 0 && <span style={{ color: '#3498db', marginLeft: '6px', fontWeight: 'bold', fontSize: '0.85rem' }}>↑{myRankingInfo.user.rankDiff}</span>}
                    {myRankingInfo.user.rankDiff < 0 && <span style={{ color: '#e74c3c', marginLeft: '6px', fontWeight: 'bold', fontSize: '0.85rem' }}>↓{Math.abs(myRankingInfo.user.rankDiff)}</span>}
                  </span>
