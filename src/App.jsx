...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      if (data) {
        setRoomData(data);
        // ホストが開始を決定した（status === 'starting' または 復帰用 'playing'）ら、試合画面に遷移
  ...
  // 待機ルームのタイマー処理（0になったらホストが開始処理をキック）
  useEffect(() => {
    let timer = null;
  ...
  // ★ Ver3.3.6 開始ボタン押下時の処理。全員が遷移してからexecuteStartOnlineMatchを実行させるため、ここではstatusを'starting'にするのみ。
  const startOnlineGameWithCpu = async () => {
    if (!roomId || !roomData || isStartingRoom) return;
    setIsStartingRoom(true); // 連打防止
  ...
  // ★ Ver3.3.6: ホスト側で全員の準備が完了したら試合をスタートさせる
  useEffect(() => {
    if (isOnlineMode && isHost && roomId && roomData?.status === 'starting') {
      ...
      if (allReady && !hasStartedMatch) {
        setHasStartedMatch(true); // 1度だけ実行
      ...
      if (snapshot.exists()) {
        ...
        if (existingPlayers.length < 4) {
          ...
          if (updatedPlayers.length === 4 && type.startsWith('rank')) {
            // ★ ホストが招待で4人揃えた時も同様に状態を starting にして全員に画面遷移を促す
