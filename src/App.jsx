...
function App() {
  ...
  // ★ Ver.1.7.4: ランク戦で4人が揃った際にホストがゲームを開始する処理
  useEffect(() => {
    if (roomId && roomData && roomData.hostId === userData.userId && currentScreen.startsWith('waiting')) {
      if (roomData.players && roomData.players.length === 4 && roomData.status === 'waiting') {
        executeStartOnlineMatch(roomId, roomData.players);
      }
    }
  }, [roomId, roomData, currentScreen, userData.userId]);
  ...
  const joinRankMatch = async () => {
    ...
    if (targetRoomId) {
      ...
      // ★ バグ修正：ゲストが勝手に executeStartOnlineMatch を呼び出さないように削除（ホストが useEffect で検知して実行する）
    } else {
  ...
  const playCards = (playerIdx, cards) => {
    ...
    const newField = { cards, strength, count: cards.length, playedBy: playerIdx, isSeq };
    setField(newField);
