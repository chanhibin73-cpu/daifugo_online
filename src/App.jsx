...
// レーティング計算ロジック（対戦相手のレーティング差分を強く反映するシステム）
const calcRating = (myRating, allRatings, myRank, finalRankings) => {
  ...
  // 相手の平均レーティングを計算して差分を加味
  let oppTotal = 0;
  let oppCount = 0;
  for (let i = 0; i < 4; i++) {
    if (i !== myRank) {
      oppTotal += allRatings[finalRankings[i]];
      oppCount++;
    }
  }
  const oppAvg = oppTotal / oppCount;
  const diff = oppAvg - myRating;
  
  // 相手の平均レートが高いほど勝った時のプラス増、負けた時のマイナス減
...
function App() {
  ...
  const [showUpdateModal, setShowUpdateModal] = useState(false); // 更新情報モーダル
  ...
  // アップデート情報表示フロー
  useEffect(() => {
    if (userData.username && userData.lastSeenVersion !== '1.1') {
  ...
  const handleCloseUpdate = () => {
    ...
    saveUserData({ ...userData, lastSeenVersion: '1.1' });
  };
  ...
  const handleRegister = (e) => {
    ...
    const updated = { ...userData, username: inputUsername.trim(), lastSeenVersion: '1.1' };
  ...
  const joinRankMatch = async () => {
    ...
    if (targetRoomId) {
      ...
      setMySlot(newSlot);
      const updatedPlayers = [...targetRoom.players, { userId: userData.userId, name: userData.username, rating: userData.rating, isCpu: false }];
  ...
  const joinRoomByCode = async () => {
    ...
    setMySlot(rData.players.length); setIsRankMatch(false); setIsOnlineMode(true); setRoomId(targetId);
    const updatedPlayers = [...rData.players, { userId: userData.userId, name: userData.username, rating: userData.rating, isCpu: false }];
  ...
  const startOnlineGameWithCpu = async () => {
    ...
    while (currentPlayers.length < 4) {
      currentPlayers.push({ userId: `cpu_${cpuCount}`, name: `CPU ${cpuCount++}`, rating: Math.floor(Math.random() * 801) + 100, isCpu: true });
    }
  ...
  // CPU自動打札 (ホストのみ実行、レーティングに比例して賢い手を打つ)
  useEffect(() => {
    ...
    if (isCurrentCpuSlot) {
      const cpuTimer = setTimeout(() => {
        ...
        let move = findBestCpuMove(cpuHand, field, isReversedEffective, playerRatings[turn]);
  ...
  const findBestCpuMove = (hand, currentField, isRev, cpuRating) => {
    ...
    // レーティング比例の思考アルゴリズム
    const isSmart = (Math.random() * 1000) < cpuRating;
    const isVerySmart = isSmart && ((Math.random() * 1000) < cpuRating);

    if (!currentField) {
      if (isVerySmart && hand.length <= 3) {
      ...
      if (isSmart) {
        for (let num in groups) { 
          if (num != 8 && getCardStrength(Number(num), isRev) < 13 && groups[num].length >= 2 && !isForbiddenFinish(groups[num])) return groups[num]; 
        }
        for (let i = 0; i < sorted.length; i++) {
          const card = sorted[i];
          if (card.num !== 0 && card.num !== 8 && getCardStrength(card.num, isRev) < 13 && groups[card.num].length === 1 && !isForbiddenFinish([card])) return [card];
        }
      }

      const eights = sorted.filter(c => c.num === 8);
      if (eights.length > 0 && !isForbiddenFinish(eights)) return [eights[0]];
      for (let num in groups) { if (groups[num].length >= 2 && !isForbiddenFinish(groups[num])) return groups[num]; }
      for (let i = 0; i < sorted.length; i++) { if (!isForbiddenFinish([sorted[i]])) return [sorted[i]]; }
      return null;
    }
    ...
    if (reqCount === 1) {
      ...
      if (isSmart && valid.length > 0) {
        const optimal = valid.find(c => {
          if (c.num === 0) return true;
          if (groups[c.num].length > 1) return false;
          if (isVerySmart && getCardStrength(c.num, isRev) >= 13 && hand.length > 3) return false;
          return true;
        });
  ...
  const handleConfirmReturnHome = async () => {
    ...
    if (isOnlineMode && roomId && roomData) {
      ...
      if (updatedPlayers[mySlot]) {
        updatedPlayers[mySlot] = { userId: `cpu_${mySlot}`, name: `CPU (代行)`, rating: userData.rating, isCpu: true };
      }
      await update(ref(rtdb, `rooms/${roomId}`), { players: updatedPlayers });
  ...
  return (
    <div className="app-outer" style={containerStyle}>
      <div className="app-container">
        ...
        {/* --- アップデート情報モーダル (Ver.1.1) --- */}
        {showUpdateModal && (
          <div className="modal-overlay">
            <div className="modal-content">
              <h3 style={{ margin: '0 0 10px 0', color: '#2c3e50' }}>🎉 アップデート情報</h3>
              <div className="rules-scroll-area" style={{ textAlign: 'left', fontSize: '12px', lineHeight: '1.5' }}>
                <p style={{ margin: '0 0 8px 0' }}>大富豪オンラインが <strong>Ver.1.1</strong> にアップデートされました！</p>
                <ul style={{ paddingLeft: '20px', margin: 0 }}>
                  <li style={{ marginBottom: '6px' }}><strong>🤖 コンピュータの思考プログラム変更:</strong> レーティングに比例してより深い思考（カードの温存など）を行うようになりました。</li>
                  <li><strong>📊 レーティングシステムの変更:</strong> 対戦相手のレーティング差を計算に強く反映するようになりました（格上に勝つと大きく上昇します）。</li>
                </ul>
              </div>
        ...
        {currentScreen === 'menu' && (
          <div className="menu-container">
            <h1 className="title">
              大富豪オンライン
              <div style={{ fontSize: '12px', color: '#7f8c8d', marginTop: '4px' }}>Ver.1.1</div>
            </h1>
        ...
        {currentScreen === 'settings' && (
          <div className="sub-screen settings-screen">
            ...
            <div style={{ marginTop: '20px', textAlign: 'center' }}>
              <button className="action-btn warning-btn" style={{ backgroundColor: '#e74c3c' }} onClick={() => setShowDeleteDataModal(true)}>
                🗑 全データを削除して初期化
              </button>
            </div>
