...
const UPDATE_HISTORY = [
  {
    version: '2.0.1',
    features: [
      '✨ UI/UXをアプデ前のものに切り替える機能を設定に追加しました。',
      '👑 ランキング画面の空白部分に現在の自分の順位とレートを表示するようにしました。',
      '🎨 ボタンの色を変更できる機能を設定に追加しました。'
    ]
  },
  {
    version: '1.9.5',
...
function App() {
  ...
  const [userData, setUserData] = useState(() => {
    ...
    if (saved) {
      try {
        ...
        // ★ Ver2.0.1: 新設定項目の初期値
        if (parsed.uiMode === undefined) parsed.uiMode = 'modern';
        if (parsed.btnColor === undefined) parsed.btnColor = '#3498db';
        return parsed;
      } catch (e) {}
    }
    return {
  ...
  return (
    <div className={"app-outer " + (userData.uiMode === 'classic' ? 'classic-ui' : '')} style={containerStyle}>
      <style>{`
        :root {
          --custom-btn-bg: ${userData.btnColor || '#3498db'};
        }
        .menu-btn, .action-btn, .play-btn {
          background-color: var(--custom-btn-bg) !important;
          border-color: var(--custom-btn-bg) !important;
        }
        .classic-ui .menu-btn, .classic-ui .action-btn, .classic-ui .game-btn, .classic-ui .play-btn, .classic-ui .pass-btn {
          border-radius: 2px !important;
          box-shadow: none !important;
        }
        .classic-ui .card-item {
      ...
      <div className="app-container">
        ...
        {currentScreen === 'ranking' && (
          <div className="sub-screen" style={{ display: 'flex', flexDirection: 'column', maxHeight: '100%' }}>
            ...
            <div className="ranking-list" style={{ flex: 1, overflowY: 'auto' }}>
              {rankingLoading ? (
            ...
            {/* ★ Ver2.0.1: ランキングの空白部分（下部）に現在の自分を表示 */}
            {!rankingLoading && (
              <div style={{ marginTop: '15px', padding: '10px', background: 'rgba(255,255,255,0.9)', borderRadius: '8px', fontWeight: 'bold', border: `2px solid ${userData.btnColor || '#3498db'}`, textAlign: 'center' }}>
                {(() => {
                  const myRankIndex = globalRankings.findIndex(user => user.userId === userData.userId);
                  if (myRankIndex !== -1) {
                    return `🏆 あなたの順位: ${myRankIndex + 1}位 (${globalRankings[myRankIndex].rating} Pt)`;
                  } else {
                    return `🏆 あなたの順位: ランク外 (${userData.rating} Pt)`;
                  }
                })()}
              </div>
            )}

            <button className="back-btn" onClick={() => setCurrentScreen('menu')} style={{ marginTop: '15px' }}>戻る</button>
          </div>
        )}
        ...
        {currentScreen === 'settings' && (
          <div className="sub-screen settings-screen">
            ...
            {/* ★ Ver2.0.1 UI/UX切り替え機能 */}
            <div className="setting-item">
              <label>UI/UXモード</label>
              <select value={userData.uiMode} onChange={(e) => saveUserData({ ...userData, uiMode: e.target.value })}>
                <option value="modern">モダン (最新)</option>
                <option value="classic">クラシック (アプデ前)</option>
              </select>
            </div>
            ...
            <div className="setting-item">
              <label>ボタンの色</label>
              <input type="color" style={{ width: '100%', height: '40px', padding: '0', border: 'none', borderRadius: '4px' }} value={userData.btnColor} onChange={(e) => saveUserData({ ...userData, btnColor: e.target.value })} />
            </div>
            ...
            <div style={{ marginTop: '15px', textAlign: 'center' }}>
              <button className="action-btn" style={{ width: '100%' }} onClick={() => setShowUpdateModal(true)}>
                📢 更新情報 (リリースノート) を確認
              </button>
            </div>

            <div style={{ marginTop: '10px', textAlign: 'center' }}>
              <button className="action-btn warning-btn" style={{ backgroundColor: '#e74c3c', borderColor: '#e74c3c', width: '100%' }} onClick={() => setShowDeleteDataModal(true)}>
                🗑 全データを削除して初期化
              </button>
            </div>
